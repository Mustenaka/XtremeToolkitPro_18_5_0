...
void CHistoryView::OnDraw(CDC* pDC)
{
	...
	int x = r.Width()/2 - XTP_DPI_X(50);
	int y = r.Height() / 2 - XTP_DPI_Y(25);

	CRect rcSquare(x, y, x + XTP_DPI_X(100), y + XTP_DPI_Y(50));

	CFont* pOldFont = pDC->SelectObject(&XTPAuxData().xtpFont);
...
void CHistoryView::OnSize(UINT nType, int cx, int cy)
{
	...
	if( m_barCaption.GetSafeHwnd()) {
		m_barCaption.MoveWindow(0, 0, cx, XTP_DPI_Y(19));
	}

	if( m_btnCaption.GetSafeHwnd()) {
		m_btnCaption.MoveWindow(cx - XTP_DPI_X(18), XTP_DPI_Y(2),
			XTP_DPI_X(16), XTP_DPI_Y(15));
	}
}
