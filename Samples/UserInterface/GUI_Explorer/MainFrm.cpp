...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_wndLogo.Create(NULL, WS_CHILD|WS_VISIBLE|SS_BITMAP|SS_CENTERIMAGE, 
		XTP_DPI(CRect(0, 0, 100, 23)), this);
	...
	rbbi.cx         =
	rbbi.cxIdeal    =
	rbbi.cxMinChild = XTP_DPI_X(100);
	rbbi.cyMinChild = XTP_DPI_Y(23);
	rbbi.wID = IDB_LOGO;
	...
	CCreateContext* pContext)
{
	...
	if (!m_wndSplitter.CreateView(0, 0, RUNTIME_CLASS(CGUI_ExplorerTree),
		XTP_DPI(CSize(300, 100)), pContext))
	{
	...
	if (!m_wndSplitter.CreateView(0, 1, RUNTIME_CLASS(CGUI_ExplorerView),
		XTP_DPI(CSize(100, 100)), pContext))
	{
		m_wndSplitter.DestroyWindow();
		return FALSE;
	}
...
void CMainFrame::InitializeAddressBar()
{
	...
	if (!m_wndAddressBar.Create(CBS_DROPDOWN | WS_CHILD,
		XTP_DPI(CRect(0,0,200,400)), this, IDC_ADDRESSBAR))
	{
	...
	m_wndAddressBar.SetFont(&XTPAuxData().xtpFont);
...
void CMainFrame::OnViewXpmode()
{
	...
	if (m_bXPMode)
		CXTPPaintManager::SetCustomTheme(new CXTPReBarPaintManager<CXTPVisualStudio2005Theme>);
	else
