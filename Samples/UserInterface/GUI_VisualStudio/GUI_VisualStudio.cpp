...
CGUI_VisualStudioApp::CGUI_VisualStudioApp() : pDocTemplate(0)
{
	EnableManifestEditor();
	m_theme = themeVS2015Light;
	strSchema.Empty();
	strColor.Empty();
}
...
void CGUI_VisualStudioApp::UpdateAllViews(CView* pSender/*=NULL*/)
{
	if (!pDocTemplate == NULL)
	{
		for (POSITION posDoc = pDocTemplate->GetFirstDocPosition(); posDoc;)
		{
			CXTPSyntaxEditDoc* pDoc = DYNAMIC_DOWNCAST(
				CXTPSyntaxEditDoc, pDocTemplate->GetNextDoc(posDoc));
