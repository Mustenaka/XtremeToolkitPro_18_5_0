...
CEclipseHost::CEclipseHost()
{
	...
	m_nHeaderHeight = m_szCaption.cy = XTP_DPI_Y(25);
}
...
void CEclipseHost::OnSize(UINT nType, int cx, int cy)
{
	...
	m_nHeaderHeight = m_szCaption.cy = szHostBar.cy + XTP_DPI_Y(1);
	...
	if (m_wndToolBar.GetSafeHwnd())
	{
		...
		if (szHostBar.cx + szToolBar.cx < cx - XTP_DPI_X(5) - XTP_DPI_X(16) - sz.cx)
		{
...
void CEclipseHost::OnPaint()
{
	...
	dc.FillSolidRect(rcTitle.left, rcTitle.bottom - XTP_DPI_Y(1), 
		rcTitle.Width(), XTP_DPI_Y(1), RGB(132, 130, 132));

	if (m_hIcon)
	{
		dc.DrawState(CPoint(rcTitle.left + XTP_DPI_X(5), rcTitle.CenterPoint().y - XTP_DPI_Y(8)),
			CSize(16, 16), m_hIcon, 0, (CBrush*)0);
	}

	CRect rcText(rcTitle.left + XTP_DPI_X(5) + XTP_DPI_X(16) + XTP_DPI_X(5), rcTitle.top, rcFill.right, rcTitle.bottom);
...
BOOL CEclipseHost::Create(CWnd* pParentWnd)
{
	return CreateEx(0, _T("STATIC"), _T(""),
		SS_NOTIFY | WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, 
		XTP_DPI(CRect(0, 0, 200, 120)), pParentWnd, 0);
}
