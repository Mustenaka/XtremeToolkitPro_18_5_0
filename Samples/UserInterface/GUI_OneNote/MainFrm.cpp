...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_paneManager.CreatePane(
		IDR_PANE_NEW, XTP_DPI(CRect(0, 0,200, 120)), xtpPaneDockRight);

	CXTPDockingPane* pPaneSearch = m_paneManager.CreatePane(
		IDR_PANE_SEARCH, XTP_DPI(CRect(0, 0, 200, 120)), xtpPaneDockLeft);
	pPaneSearch->Hide();
	...
	pTabPaintManager->GetAppearanceSet()->m_rcHeaderMargin.SetRect(XTP_DPI_X(2), XTP_DPI_Y(6), XTP_DPI_X(2), 0);
	pTabPaintManager->m_rcClientMargin.SetRect(XTP_DPI_X(2), XTP_DPI_Y(2), XTP_DPI_X(2), XTP_DPI_Y(2));
	pTabPaintManager->m_rcButtonMargin.SetRect(XTP_DPI_X(4), XTP_DPI_Y(1), XTP_DPI_X(4), XTP_DPI_Y(1));
...
int CMainFrame::OnCreateControl(LPCREATECONTROLSTRUCT lpCreateControl)
{
	...
	if (lpCreateControl->nID == ID_HELP_QUESTION)
	{
		...
		pComboQuestion->SetWidth(XTP_DPI_X(150));
...
void CMainFrame::CreateTaskPanel()
{
	...
	pGroupNew->GetInnerMargins().SetRect(0, XTP_DPI_Y(5), 0, 0);
	...
	pTextItem->GetMargins().SetRect(XTP_DPI_X(20), 0, 0, XTP_DPI_Y(5));
	...
	m_wndButtonAutoRun.Create(_T("Show at Startup"), WS_CHILD|BS_AUTOCHECKBOX|BS_NOTIFY,
		XTP_DPI(CRect(0, 0, 300, 20)), &m_wndTaskPanel, IDC_BUTTON_AUTORUN);
	...
	pItemButton->SetSize(XTP_DPI(CSize(0, 20)));
	pItemButton->GetMargins().SetRect(0, XTP_DPI_Y(15), 0, 0);
