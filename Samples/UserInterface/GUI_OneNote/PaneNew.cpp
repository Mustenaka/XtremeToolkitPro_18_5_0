...
void CPaneNew::OnPaint()
{
	...
	int nTop = XTP_DPI_Y(20);


	for (int i = 0; i < _countof(tasks); i++)
	{
		XTPImageManager()->GetImage(tasks[i].nID)->Draw(&dc, CPoint(XTP_DPI_X(17), nTop - XTP_DPI_Y(2)));
		dc.SetTextColor(RGB(0, 0, 255));
		CRect rcUrl(XTP_DPI_X(37), nTop, rc.Width() - XTP_DPI_X(10), nTop + nTextHeight);
		dc.DrawText(tasks[i].lpUrl, rcUrl, DT_SINGLELINE);

		nTop = nTop + nTextHeight + XTP_DPI_Y(3);
		dc.SetTextColor(GetSysColor(COLOR_BTNTEXT));

		CRect rcDesc(XTP_DPI_X(37), nTop, rc.Width() - XTP_DPI_X(10), nTop);
		dc.DrawText(tasks[i].lpDesc, rcDesc, DT_WORDBREAK|DT_CALCRECT);
		dc.DrawText(tasks[i].lpDesc, rcDesc, DT_WORDBREAK);

		nTop = rcDesc.bottom + XTP_DPI_Y(13);
	}
}
