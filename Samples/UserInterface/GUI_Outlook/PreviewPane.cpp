...
#define CAPT_MAX_HEIGHT XTP_DPI_Y(35)
#define CAPT_MIN_HEIGHT XTP_DPI_Y(5)

#define TEXT_BOLD_LEFT  XTP_DPI_X(4)
#define TEXT_NORM_LEFT  XTP_DPI_X(75)
#define TEXT_LINE_ONE   XTP_DPI_Y(4)
#define TEXT_LINE_TWO   XTP_DPI_Y(18)
...
void CPreviewPane::OnDraw(CDC* pDC)
{
	...
	if (theme == xtpThemeOffice2003)
	{
		...
		pDC->FillSolidRect(m_rcCaption.left, m_rcCaption.bottom - XTP_DPI_Y(1), 
			m_rcCaption.right, XTP_DPI_Y(1), GetXtremeColor(XPCOLOR_FRAME));
	}
	else
	{
		...
		rc.DeflateRect(XTP_DPI_X(1), XTP_DPI_Y(1), theme == xtpThemeNativeWinXP ? 0 : XTP_DPI_X(1), 0);
...
int CPreviewPane::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_richEditCtrl.SetFont( &XTPAuxData().xtpFont );
