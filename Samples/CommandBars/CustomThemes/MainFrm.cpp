...
void CMainFrame::SetTheme(XTPPaintTheme theme)
{
	...
	if ((m_bOffice2007Frame == 1) || ((XTPPaintManager()->BaseTheme() == xtpThemeResource || XTPPaintManager()->BaseTheme() == xtpThemeRibbon) &&
		(m_bOffice2007Frame == 2)))
	{
...
void CMainFrame::OnSwitchTheme(UINT nTheme)
{
	switch (nTheme)
	{
		case ID_THEME_DEFAULT: SetTheme(xtpThemeOffice2000); break;
		case ID_THEME_OFFICE_XP: SetTheme(xtpThemeOfficeXP); break;
		case ID_THEME_OFFICE_2003: SetTheme(xtpThemeOffice2003); break;
		case ID_THEME_WINDOWS_XP: SetTheme(xtpThemeNativeWinXP); break;
		case ID_THEME_RIBBON: SetTheme(xtpThemeRibbon); break;
		case ID_THEME_OFFICE2007: SetTheme(xtpThemeResource); break;
		case ID_THEME_VISUALSTUDIO2008: SetTheme(xtpThemeVisualStudio2008); break;
...
void CMainFrame::OnUpdateViewOffice2007Frame(CCmdUI* pCmdUI)
{
	...
	pCmdUI->Enable((XTPPaintManager()->GetCurrentTheme() == xtpThemeResource || XTPPaintManager()->GetCurrentTheme() == xtpThemeRibbon) ? TRUE : FALSE);
...
int CMainFrame::OnCreateControl(LPCREATECONTROLSTRUCT lpCreateControl)
{
	if (lpCreateControl->bToolBar == FALSE)
	{
		...
		if (lpCreateControl->controlType == xtpControlButton && lpCreateControl->nID == ID_EDIT_FIND)
		{
			...
			pComboFind->SetWidth(XTP_DPI_X(150));
		...
		if (lpCreateControl->nID == ID_PARA_INDENT)
		{
			...
			pControl->SetWidth(XTP_DPI_X(133));
	...
	else
	{
		...
		if (lpCreateControl->nID == ID_COMBO_MASKED)
		{
			...
			pComboMasked->SetWidth(XTP_DPI_X(130));
		...
		if (lpCreateControl->nID == ID_COMBO_OWNERDRAW)
		{
			...
			pComboCustomDraw->SetWidth(XTP_DPI_X(130));
			pComboCustomDraw->SetDropDownWidth(XTP_DPI_X(230));
		...
		if (lpCreateControl->nID == ID_COMBO_POPUP)
		{
			...
			pComboPopup->SetWidth(XTP_DPI_X(130));
		...
		if (lpCreateControl->nID == ID_BUTTONLINE && pToolBar->GetBarID() == IDR_CUSTOM)
		{
			...
			CXTPPopupBar* pLineBar = (CXTPPopupBar*)CXTPPopupToolBar::CreatePopupToolBar(GetCommandBars());
		...
		if (lpCreateControl->nID == ID_BUTTONSTYLE && pToolBar->GetBarID() == IDR_CUSTOM)
		{
			...
			CXTPPopupBar* pStyleBar = (CXTPPopupBar*)CXTPPopupToolBar::CreatePopupToolBar(GetCommandBars());
		...
		if (lpCreateControl->nID == ID_BUTTONARROW && pToolBar->GetBarID() == IDR_CUSTOM)
		{
			...
			CXTPPopupBar* pArrowBar = (CXTPPopupBar*)CXTPPopupToolBar::CreatePopupToolBar(GetCommandBars());
		...
		if (lpCreateControl->nID == ID_BUTTON_TEXT && pToolBar->GetBarID() == IDR_CUSTOM)
		{
			...
			CXTPPopupBar* pColorBar = (CXTPPopupBar*)CXTPPopupToolBar::CreatePopupToolBar(GetCommandBars());
