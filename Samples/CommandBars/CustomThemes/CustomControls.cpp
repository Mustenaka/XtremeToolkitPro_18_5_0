...
CSize CControlButtonLine::GetSize(CDC* pDC)
{
	...
	return XTP_DPI(CSize(160, max(sz.cy, 16)));
}
...
void CControlButtonLine::Draw(CDC* pDC)
{
	...
	int nWidth = (int)m_dWidth + XTP_DPI_X(1);

	CString str;
	str.Format(_T("%2.2f"), m_dWidth);
	CRect rcText(rc); rcText.DeflateRect(XTP_DPI_X(2), XTP_DPI_Y(2));
	pDC->DrawText(str, rcText, DT_SINGLELINE|DT_VCENTER );

	CSize sz = pDC->GetTextExtent(str);

	CRect rcButton = CRect(rc.left + sz.cx + XTP_DPI_X(8), rc.CenterPoint().y - nWidth / 2, 
		rc.right - XTP_DPI_X(3), rc.CenterPoint().y - nWidth / 2 + nWidth);
...
void CControlButtonStyle::Draw(CDC* pDC)
{
	...
	CRect rcButton = CRect(rc.left + XTP_DPI_X(3), rc.CenterPoint().y - nWidth / 2,
		rc.right - XTP_DPI_X(3), rc.CenterPoint().y - nWidth / 2 + nWidth);
...
void CControlButtonArrow::Draw(CDC* pDC)
{
	...
	CRect rc = m_rcControl;
	CRect rcButton = CRect(rc.left + XTP_DPI_X(3), rc.CenterPoint().y, rc.right - XTP_DPI_X(3), rc.CenterPoint().y);

	XTPPaintManager()->Line(pDC, rcButton.left, rcButton.top, rcButton.right, rcButton.top, COLOR_BTNTEXT);

	if (m_nStyle == 2 || m_nStyle == 4)
	{
		XTPPaintManager()->Line(pDC, rcButton.left, rcButton.top, rcButton.left + XTP_DPI_X(8), rcButton.top - XTP_DPI_Y(4), COLOR_BTNTEXT);
		XTPPaintManager()->Line(pDC, rcButton.left, rcButton.top, rcButton.left + XTP_DPI_X(8), rcButton.top + XTP_DPI_Y(4), COLOR_BTNTEXT);
	}
	if (m_nStyle == 3 || m_nStyle == 4)
	{
		XTPPaintManager()->Line(pDC, rcButton.right - XTP_DPI_X(8), rcButton.top - XTP_DPI_Y(4), rcButton.right, rcButton.top, COLOR_BTNTEXT);
		XTPPaintManager()->Line(pDC, rcButton.right - XTP_DPI_X(8), rcButton.top + XTP_DPI_Y(4), rcButton.right, rcButton.top, COLOR_BTNTEXT);
	}
