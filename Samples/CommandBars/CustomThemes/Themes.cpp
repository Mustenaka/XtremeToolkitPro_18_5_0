...
CSize CCustomGrayTheme::DrawCommandBarGripper(CDC* pDC, CXTPCommandBar* pBar, BOOL bDraw)
{
	...
	if (pBar->IsDialogBar())
	{
		...
		if (pDC && bDraw)
		{
			...
			{
				pen.Color(clrGripper[i % 3]);
				pDC->MoveTo(XTP_DPI_X(3), i);
				pDC->LineTo(XTP_DPI_X(8), i);
			}
		}
		return sz;
	...
	else if (pBar->GetPosition() == xtpBarRight || pBar->GetPosition() == xtpBarLeft)
	{
		if (pDC && bDraw)
		{
			...
			for (int i = 3; i < rc.right - XTP_DPI_X(4); i++)
			{
				pen.Color(clrGripper[i % 3]);
				pDC->MoveTo(i, XTP_DPI_Y(3));
				pDC->LineTo(i, XTP_DPI_Y(8));
			}
		}
	...
	else if (pBar->GetPosition() == xtpBarTop || pBar->GetPosition() == xtpBarBottom)
	{
		if (pDC && bDraw)
		{
			for (int i = 3; i < rc.bottom - XTP_DPI_Y(4); i++)
			{
				pen.Color(clrGripper[i % 3]);
				pDC->MoveTo(XTP_DPI_X(3), i);
				pDC->LineTo(XTP_DPI_X(8), i);
...
void CCustomGradientTheme::AdjustExcludeRect(CRect& rc, CXTPControl* pControl, BOOL bVertical)
{
	...
	{
		if (bVertical)
			rc.DeflateRect(-XTP_DPI_X(1), 0); else rc.DeflateRect(0, -XTP_DPI_Y(1));
	} else rc.InflateRect(XTP_DPI_X(3), 0);
...
{
	pDC->RoundRect(rect, XTP_DPI(CPoint(6, 6)));
	...
	pDC->MoveTo(nLeft + XTP_DPI_X(2), nBottom - XTP_DPI_Y(4));
	pDC->LineTo(nLeft + XTP_DPI_X(2), nTop + XTP_DPI_Y(2));
	pDC->LineTo(nRight - XTP_DPI_X(3), nTop + XTP_DPI_Y(2));
	pDC->SetPixel(nLeft + XTP_DPI_X(3), nTop + XTP_DPI_Y(3), m_crHilight);

	pDC->SelectObject(&m_penLiteShadow);
	pDC->MoveTo(nLeft + XTP_DPI_X(3), nBottom - XTP_DPI_Y(3));
	pDC->LineTo(nRight - XTP_DPI_X(4), nBottom - XTP_DPI_Y(3));
	pDC->LineTo(nRight - XTP_DPI_X(4), nBottom - XTP_DPI_Y(4));
	pDC->LineTo(nRight - XTP_DPI_X(3), nBottom - XTP_DPI_Y(4));
	pDC->LineTo(nRight - XTP_DPI_X(3), nTop + XTP_DPI_Y(2));

	pDC->SelectObject(&m_penShadow);
	pDC->MoveTo(nLeft + XTP_DPI_X(3), nBottom - XTP_DPI_Y(2));
	pDC->LineTo(nRight - XTP_DPI_X(3), nBottom - XTP_DPI_Y(2));
	pDC->LineTo(nRight - XTP_DPI_X(3), nBottom - XTP_DPI_Y(3));
	pDC->LineTo(nRight - XTP_DPI_X(2), nBottom - XTP_DPI_Y(3));
	pDC->LineTo(nRight - XTP_DPI_X(2), nTop + XTP_DPI_Y(2));

	pDC->SetPixel(nLeft, nTop + XTP_DPI_Y(2), m_crDarkDkShadow);
	pDC->SetPixel(nLeft + XTP_DPI_X(2), nTop, m_crDarkDkShadow);
	pDC->SetPixel(nRight - XTP_DPI_X(3), nTop, m_crDarkDkShadow);
	pDC->SetPixel(nRight - XTP_DPI_X(1), nTop + XTP_DPI_Y(2), m_crDarkDkShadow);
	pDC->SetPixel(nRight - XTP_DPI_X(1), nBottom - XTP_DPI_Y(3), m_crDarkDkShadow);
	pDC->SetPixel(nRight - XTP_DPI_X(3), nBottom - XTP_DPI_Y(1), m_crDarkDkShadow);
	pDC->SetPixel(nLeft + XTP_DPI_X(2), nBottom - XTP_DPI_Y(1), m_crDarkDkShadow);
	pDC->SetPixel(nLeft, nBottom - XTP_DPI_Y(3), m_crDarkDkShadow);

	pDC->SetPixel(nLeft + XTP_DPI_X(1), nTop + XTP_DPI_Y(2), m_crLiteShadow);
	pDC->SetPixel(nLeft + XTP_DPI_X(2), nTop + XTP_DPI_Y(1), m_crLiteShadow);
	pDC->SetPixel(nRight - XTP_DPI_X(3), nTop + XTP_DPI_Y(1), m_crLiteShadow);
	pDC->SetPixel(nRight - XTP_DPI_X(2), nTop + XTP_DPI_Y(2), m_crLiteShadow);
...
{
	pDC->SetPixel(rect.left, rect.top + XTP_DPI_Y(2), m_crDarkDkShadow);
	pDC->SetPixel(rect.left + XTP_DPI_X(2), rect.top, m_crDarkDkShadow);
	pDC->SetPixel(rect.right - XTP_DPI_X(3), rect.top, m_crDarkDkShadow);
	pDC->SetPixel(rect.right - XTP_DPI_X(1), rect.top + XTP_DPI_Y(2), m_crDarkDkShadow);
	pDC->SetPixel(rect.right - XTP_DPI_X(1), rect.bottom - XTP_DPI_Y(3), m_crDarkDkShadow);
	pDC->SetPixel(rect.right - XTP_DPI_X(3), rect.bottom - XTP_DPI_Y(1), m_crDarkDkShadow);
	pDC->SetPixel(rect.left + XTP_DPI_X(2), rect.bottom - XTP_DPI_Y(1), m_crDarkDkShadow);
	pDC->SetPixel(rect.left, rect.bottom - XTP_DPI_Y(3), m_crDarkDkShadow);
}   // DrawCornerShadows
...
void CCustomBevelTheme::CreatePens()
{
	...
	m_penLiteShadow.CreatePen(PS_SOLID, XTP_DPI_X(1), m_crLiteShadow);
	m_penShadow.CreatePen(PS_SOLID, XTP_DPI_X(1), m_crShadow);
	m_penHilight.CreatePen(PS_SOLID, XTP_DPI_X(1), m_crHilight);
}   // CreatePens
...
void CCustomBevelTheme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition)
{
	...
	if (!bEnabled)
	{
		...
		CPen penFrame(PS_SOLID, XTP_DPI_X(1), ::GetSysColor(COLOR_WINDOWFRAME));
		CPen *pOldPen = (CPen *)pDC->SelectObject(&m_penShadow);

		pDC->RoundRect(rc, XTP_DPI(CPoint(6, 6)));
...
void CCustomBevelTheme::AdjustExcludeRect(CRect& rc, CXTPControl* pControl, BOOL bVertical)
{
	...
	else rc.InflateRect(-XTP_DPI_X(3), XTP_DPI_Y(3));
...
CSize CCustomIcyTheme::DrawCommandBarGripper(CDC* pDC, CXTPCommandBar* pBar, BOOL bDraw)
{
	...
	if (pBar->IsDialogBar())
	{
		...
		if (pDC && bDraw)
		{
			for (int y = XTP_DPI_Y(5); y < sz.cy - XTP_DPI_Y(2); y += XTP_DPI_Y(4))
			{
				pDC->FillSolidRect(CRect(XTP_DPI_X(4) + XTP_DPI_X(2), y + XTP_DPI_Y(1), XTP_DPI_X(6) + XTP_DPI_X(2), y + XTP_DPI_Y(3)), GetXtremeColor(COLOR_BTNHIGHLIGHT));
				pDC->FillSolidRect(CRect(XTP_DPI_X(3) + XTP_DPI_X(2), y, XTP_DPI_X(5) + XTP_DPI_X(2), y + XTP_DPI_Y(2)), DarkenColor(60, GetXtremeColor(COLOR_3DFACE)));
			}
		}
		return sz;
	...
	{
		if (!pDC || !bDraw) return CSize(0, XTP_DPI_Y(8) + XTP_DPI_X(1));
		for (int x = XTP_DPI_X(5); x < rc.Width() - XTP_DPI_X(8); x += XTP_DPI_X(4))
		{
			pDC->FillSolidRect(CRect(x + XTP_DPI_X(1), XTP_DPI_Y(4) + XTP_DPI_Y(3), x + XTP_DPI_X(3), XTP_DPI_Y(6) + XTP_DPI_Y(3)), GetXtremeColor(COLOR_BTNHIGHLIGHT));
			pDC->FillSolidRect(CRect(x, XTP_DPI_Y(3) + XTP_DPI_Y(3), x + XTP_DPI_X(2), XTP_DPI_Y(5) + XTP_DPI_Y(3)), DarkenColor(60, GetXtremeColor(COLOR_3DFACE)));
		}
	}
	else if (pBar->GetPosition() == xtpBarTop || pBar->GetPosition() == xtpBarBottom)
	{
		if (!pDC || !bDraw) return CSize(XTP_DPI_X(6) + XTP_DPI_X(2) - XTP_DPI_X(2), 0);
		for (int y = XTP_DPI_Y(5); y < rc.Height() - XTP_DPI_Y(8); y += XTP_DPI_Y(4))
		{
			pDC->FillSolidRect(CRect(XTP_DPI_X(4) + XTP_DPI_X(2), y + XTP_DPI_Y(1), XTP_DPI_X(6) + XTP_DPI_X(2), y + XTP_DPI_Y(3)), GetXtremeColor(COLOR_BTNHIGHLIGHT));
			pDC->FillSolidRect(CRect(XTP_DPI_X(3) + XTP_DPI_X(2), y, XTP_DPI_X(5) + XTP_DPI_X(2), y + XTP_DPI_Y(2)), DarkenColor(60, GetXtremeColor(COLOR_3DFACE)));
		}
...
CSize CCustomIcyTheme::DrawCommandBarSeparator(CDC* pDC, CXTPCommandBar* pBar, CXTPControl* pControl, BOOL bDraw)
{
	...
	if (pBar->GetPosition() == xtpBarRight || pBar->GetPosition() == xtpBarLeft)
	{
		if (!pControl->GetWrap())
		{
			Line(pDC, rcRow.left + XTP_DPI_X(4), rcControl.top - XTP_DPI_Y(4), rcRow.right - XTP_DPI_X(4), rcControl.top - XTP_DPI_Y(4), XPCOLOR_3DSHADOW);
			Line(pDC, rcRow.left + XTP_DPI_X(5), rcControl.top - XTP_DPI_Y(3), rcRow.right - XTP_DPI_X(3), rcControl.top - XTP_DPI_Y(3), COLOR_3DHIGHLIGHT);
		}
		else
		{
			Line(pDC, rcRow.right + XTP_DPI_X(2), rcRow.top + XTP_DPI_Y(4), rcRow.right + XTP_DPI_X(2), rcRow.bottom, XPCOLOR_3DSHADOW);
			Line(pDC, rcRow.right + XTP_DPI_X(3), rcRow.top + XTP_DPI_Y(5), rcRow.right + XTP_DPI_X(3), rcRow.bottom, COLOR_3DHIGHLIGHT);
		}
	...
	} else if (pBar->GetPosition() == xtpBarTop || pBar->GetPosition() == xtpBarBottom)
	{
		if (!pControl->GetWrap())
		{
			Line(pDC, rcControl.left - XTP_DPI_X(4), rcRow.top + XTP_DPI_Y(4), rcControl.left - XTP_DPI_X(4), rcRow.bottom - XTP_DPI_Y(4), XPCOLOR_3DSHADOW);
			Line(pDC, rcControl.left - XTP_DPI_X(3), rcRow.top + XTP_DPI_Y(5), rcControl.left - XTP_DPI_X(3), rcRow.bottom - XTP_DPI_Y(3), COLOR_3DHIGHLIGHT);
		}
		else
		{
			Line(pDC, rcRow.left + XTP_DPI_X(4), rcRow.top - XTP_DPI_Y(4), rcRow.right, rcRow.top - XTP_DPI_Y(4), XPCOLOR_3DSHADOW);
			Line(pDC, rcRow.left + XTP_DPI_X(5), rcRow.top - XTP_DPI_Y(3), rcRow.right, rcRow.top - XTP_DPI_Y(3), COLOR_3DHIGHLIGHT);
		}
	}
...
void CCustomIcyTheme::FillCommandBarEntry(CDC* pDC, CXTPCommandBar* pBar)
{
	...
	else if (pBar->GetPosition() == xtpBarTop || pBar->GetPosition() == xtpBarBottom )
	{
		...
		if (pWnd)
		{
			...
			XTPDrawHelpers()->ExcludeCorners(pDC, CRect(rc.left + XTP_DPI_X(3), rc.top + XTP_DPI_Y(1), rc.right, rc.bottom - XTP_DPI_Y(1)));
		}
		...
		pDC->FillSolidRect(rc.left + XTP_DPI_X(5), rc.bottom - XTP_DPI_Y(1), rc.Width() - XTP_DPI_X(7), XTP_DPI_Y(1), DarkenColor(60, GetXtremeColor(COLOR_3DFACE)));
		pDC->SelectClipRgn(0);
	}
	else if (pBar->GetPosition() == xtpBarLeft || pBar->GetPosition() == xtpBarRight )
	{
		...
		if (pWnd)
		{
			...
			XTPDrawHelpers()->ExcludeCorners(pDC, CRect(rc.left + XTP_DPI_X(1), rc.top + XTP_DPI_Y(3), 
		...
		pDC->FillSolidRect(rc.right - XTP_DPI_X(1), rc.top + XTP_DPI_Y(5), 
			XTP_DPI_X(1), rc.Height() - XTP_DPI_Y(7), DarkenColor(60, GetXtremeColor(COLOR_3DFACE)));
