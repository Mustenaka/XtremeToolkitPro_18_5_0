...
: m_curLanguage(0)
{
	m_bIsWinNT = XTPSystemVersion()->IsWinNT4OrGreater();
	m_bIsWin2000OrLater = XTPSystemVersion()->IsWin2KOrGreater();
...
#define LANGID_GERMAN   MAKELANGID(LANG_GERMAN, SUBLANG_GERMAN)
#define LANGID_JAPANESE MAKELANGID(LANG_JAPANESE, SUBLANG_DEFAULT)
#define LANGID_ARABIC   MAKELANGID(LANG_ARABIC, SUBLANG_ARABIC_SAUDI_ARABIA)
#define LANGID_RUSSIAN  MAKELANGID(LANG_RUSSIAN, SUBLANG_DEFAULT)
...
BOOL CScribbleApp::InitInstance()
{
	...
	UINT curLanguage = GetProfileInt(_T("Settings"), _T("Current Language"), LANG_ENGLISH);
...
{
	if (::IsValidLocale(LANGID_ENGLISH, LCID_INSTALLED))
	{
		SetLocale(LANGID_ENGLISH, _T("English.1252"), NULL, FALSE);
	}
	else
	{
		AfxMessageBox(_T("English language support is not installed on your operating system."));
	}
}

void CScribbleApp::OnUpdateLanguageEnglish(CCmdUI* pCmdUI)
{
	pCmdUI->SetCheck(m_curLanguage == LANGID_ENGLISH);
}

void CScribbleApp::OnLanguageGerman()
{
	if (::IsValidLocale(LANGID_GERMAN, LCID_INSTALLED))
	{
		SetLocale(LANGID_GERMAN, _T("German.1252"), _T("Scribble.ResourceDe"), FALSE);
	}
	else
	{
		AfxMessageBox(_T("German language support is not installed on your operating system."));
	}
}

void CScribbleApp::OnUpdateLanguageGerman(CCmdUI* pCmdUI)
{
	pCmdUI->SetCheck(m_curLanguage == LANGID_GERMAN);
}

void CScribbleApp::OnLanguageJapanese()
{
	if (::IsValidLocale(LANGID_JAPANESE, LCID_INSTALLED))
	{
		SetLocale(LANGID_JAPANESE, _T("Japanese.932"), _T("Scribble.ResourceJa"), FALSE);
	}
	else
	{
		AfxMessageBox(_T("Japanese language support is not installed on your operating system."));
	}
}

void CScribbleApp::OnUpdateLanguageJapanese(CCmdUI* pCmdUI)
{
	pCmdUI->SetCheck(m_curLanguage == LANGID_JAPANESE);
	pCmdUI->Enable((IsUnicodeAndLayoutSupport() && m_bIsWinNT) || (GetSystemDefaultLangID() == 0x411));
}

void CScribbleApp::OnLanguageArabic()
{
	if (::IsValidLocale(LANGID_ARABIC, LCID_INSTALLED))
	{
		SetLocale(LANGID_ARABIC, _T("Arabic.1256"), _T("Scribble.ResourceArSa"), TRUE);
	}
	else
	{
		AfxMessageBox(_T("Arabic language support is not installed on your operating system."));
	}
}

void CScribbleApp::OnUpdateLanguageArabic(CCmdUI* pCmdUI)
{
	pCmdUI->SetCheck(m_curLanguage == LANGID_ARABIC);
	pCmdUI->Enable((IsUnicodeAndLayoutSupport() && m_bIsWinNT) || (GetSystemDefaultLangID() == 0x401));
}

void CScribbleApp::OnLanguageRussian()
{
	if (::IsValidLocale(LANGID_RUSSIAN, LCID_INSTALLED))
	{
		SetLocale(LANGID_RUSSIAN, _T("Russian"), _T("Scribble.ResourceRu"), FALSE);
	}
	else
	{
		AfxMessageBox(_T("Russian language support is not installed on your operating system."));
	}
}

void CScribbleApp::OnUpdateLanguageRussian(CCmdUI* pCmdUI)
{
	pCmdUI->SetCheck(m_curLanguage == LANGID_RUSSIAN);
