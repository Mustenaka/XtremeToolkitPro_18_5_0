...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
	...
	ON_COMMAND(ID_VIEW_THEME_OFFICE2013WORD, OnViewThemeOffice2013)
	ON_UPDATE_COMMAND_UI(ID_VIEW_THEME_OFFICE2013WORD, OnUpdateViewThemeOffice2013)
	//}}AFX_MSG_MAP
...
int CMainFrame::OnCreateControl(LPCREATECONTROLSTRUCT lpCreateControl)
{
	...
	if (lpCreateControl->nID == ID_EDIT_FONT)
	{
		...
		pComboFont->SetWidth(XTP_DPI_X(145));
	...
	if (lpCreateControl->nID == ID_GALLERY_FONTTEXTCOLOR)
	{
		...
		pControlGallery->SetControlSize(7, 10, XTP_DPI(CSize(17, 17)));
...
void CMainFrame::OnViewThemeOffice2013()
{
...
void CMainFrame::OnUpdateViewThemeOffice2013(CCmdUI* pCmdUI)
{
	pCmdUI->SetCheck(GetCommandBars()->GetPaintManager()->GetCurrentTheme() == xtpThemeOffice2013);
}

void CMainFrame::OnViewTheme(UINT nTheme)
{
	EnableFrameTheme((nTheme >= ID_VIEW_THEME_OFFICE2007) ? GetCommandBars() : NULL);

	if (nTheme == ID_VIEW_THEME_OFFICE2007 || 
		nTheme == ID_VIEW_THEME_RIBBON)
	{
...
{
	pCmdUI->SetCheck(XTPPaintTheme(pCmdUI->m_nID - ID_VIEW_THEME_OFFICE2000) == GetCommandBars()->GetPaintManager()->GetCurrentTheme());
}
