...
void CGalleryItemStyleSet::Draw(CDC* pDC, CXTPControlGallery* pGallery, CRect rcItem, BOOL bEnabled, BOOL bSelected, BOOL bPressed, BOOL bChecked)
{
	...
	int cx = pPaintManager->GetPopupBarGripperWidth(pGallery->GetParent()) - XTP_DPI_X(1);
	pPaintManager->DrawPopupBarGripper(pDC, rcItem.left - XTP_DPI_X(1), rcItem.top, cx + XTP_DPI_X(1), rcItem.Height());
	...
	{
		CRect rcCheck(rcItem.left + XTP_DPI_X(1), rcItem.top + XTP_DPI_Y(1), rcItem.left + cx - XTP_DPI_X(2), rcItem.bottom - XTP_DPI_Y(1));
...
void CGalleryItemThemeColors::Draw(CDC* pDC, CXTPControlGallery* pGallery, CRect rcItem, BOOL bEnabled, BOOL bSelected, BOOL bPressed, BOOL bChecked)
{
	...
	CRect rcPreview(rcItem.left + XTP_DPI_X(2), rcItem.top + XTP_DPI_Y(2), rcItem.left + XTP_DPI_X(2) + XTP_DPI_X(108), rcItem.bottom - XTP_DPI_Y(2));
	...
	rcColor.DeflateRect(XTP_DPI_X(2), XTP_DPI_Y(2));
	...
	for (int i = 0; i < _countof(nOrder); i++)
	{
		...
		rcColor.OffsetRect(rcColor.Width() + XTP_DPI_X(1), 0);
	}

	CRect rcText(rcItem);
	rcText.left += XTP_DPI_X(120);
	pDC->DrawText(m_strCaption, rcText, DT_END_ELLIPSIS|DT_SINGLELINE|DT_VCENTER);
}
...
void CGalleryItemThemeFonts::Draw(CDC* pDC, CXTPControlGallery* pGallery, CRect rcItem, BOOL bEnabled, BOOL bSelected, BOOL bPressed, BOOL bChecked)
{
	...
	CRect rcPreview(rcItem.left + XTP_DPI_X(6), rcItem.top + XTP_DPI_Y(6),
		rcItem.left + XTP_DPI_X(6) + XTP_DPI_X(43), rcItem.bottom - XTP_DPI_Y(6));
	pDC->Draw3dRect(rcPreview, RGB(97, 108, 129), RGB(97, 108, 129));
	rcPreview.DeflateRect(XTP_DPI_X(1), XTP_DPI_Y(1));
	...
	CRect rcFontPreviewBody(rcPreview.left + XTP_DPI_X(5), rcPreview.top, rcPreview.right, rcPreview.bottom - XTP_DPI_Y(5));
	CXTPFontDC fnt(pDC, &fntBody);
	pDC->DrawText(_T("A"), rcFontPreviewBody, DT_BOTTOM | DT_SINGLELINE);

	CRect rcFontPreviewBodyText(rcItem.left + XTP_DPI_X(70), rcItem.top + XTP_DPI_Y(17), rcItem.right, rcItem.bottom);
	pDC->DrawText(fontPreview.lpszBody, rcFontPreviewBodyText, DT_TOP | DT_SINGLELINE);

	CRect rcFontPreviewHeadings(rcPreview.left + XTP_DPI_X(20), rcPreview.top, rcPreview.right, rcPreview.bottom - XTP_DPI_Y(5));
	fnt.SetFont(&fntHeadings);
	pDC->DrawText(_T("a"), rcFontPreviewHeadings, DT_BOTTOM | DT_SINGLELINE);

	CRect rcFontPreviewHeadingsText(rcItem.left + XTP_DPI_X(70), rcItem.top + XTP_DPI_Y(37), rcItem.right, rcItem.bottom);
...
CGalleryItemFontColor::CGalleryItemFontColor(COLORREF clr, LPCTSTR lpszCaption, BOOL bTopMargin /*= TRUE*/, BOOL bBotttomMargin /*= TRUE*/)
{
	...
	SetSize(XTP_DPI(CSize(17, 13 + (bTopMargin ? 2 : 0) + (bBotttomMargin ? 2 : 0))));
}
...
{
	rcItem.DeflateRect(XTP_DPI_X(2), 0);
	if (m_bTopMargin)
		rcItem.DeflateRect(0, XTP_DPI_Y(2), 0, 0);
	
	if (m_bBottomMargin)
		rcItem.DeflateRect(0, 0, 0, XTP_DPI_Y(2));
	...
	{
		if (m_bTopMargin) pDC->FillSolidRect(rcItem.left, rcItem.top, rcItem.Width(), XTP_DPI_Y(1), RGB(197, 197, 197));
		pDC->FillSolidRect(rcItem.left, rcItem.top, XTP_DPI_X(1), rcItem.Height(), RGB(197, 197, 197));
		pDC->FillSolidRect(rcItem.right - XTP_DPI_X(1), rcItem.top, XTP_DPI_X(1), rcItem.Height(), RGB(197, 197, 197));
		if (m_bBottomMargin) pDC->FillSolidRect(rcItem.left, rcItem.bottom - XTP_DPI_Y(1), rcItem.Width(), XTP_DPI_Y(1), RGB(197, 197, 197));
	}
}
