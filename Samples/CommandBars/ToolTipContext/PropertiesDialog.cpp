...
BOOL CPropertiesDialog::OnInitDialog() 
{
	...
	m_moduleContext.SetMaxTipWidth(XTP_DPI_X(200));
...
CString CreateHTMLTool(LPCTSTR strCaption, LPCTSTR strDesc)
{
	...
	strHTML.Format(_T("<table cellspacing=4 cellpadding=0 border=0><tr><td><b>%s</b></td></tr>")
		_T("<tr><td><table cellspacing=4 cellpadding=0 border=0><tr><td width=6></td><td>%s</td><td width=6></td></tr></table></td></tr>")
...
CString CreateMarkupTool(LPCTSTR strCaption, LPCTSTR strDesc)
{
	...
	strHTML.Format(
			_T("<StackPanel Margin='1' MaxWidth='200'>")
				...
				_T("<TextBlock Padding='9, 6, 9, 7' TextWrapping='Wrap'>%s</TextBlock>")
...
void CPropertiesDialog::UpdateCommandBarToolTips()
{
	...
	CString shortcut, str;
	
	if (pCommandBars->GetToolTipContext()->GetStyle() == xtpToolTipMarkup)
	{
		if(m_bToolBarAccelTips)
		{
			pCommandBars->GetShortcutManager()->FindDefaultAccelerator(ID_FILE_NEW, shortcut);
			str.Format(_T("<TextBlock FontWeight='Bold' Text='New (%s)'/>"), shortcut);
		}
		pCommandBars->GetActions()->FindAction(ID_FILE_NEW)->SetTooltip(m_bToolBarAccelTips ? str : _T("<TextBlock FontWeight='Bold' Text='New'/>"));

		if(m_bToolBarAccelTips)
		{
			pCommandBars->GetShortcutManager()->FindDefaultAccelerator(ID_FILE_OPEN, shortcut);
			str.Format(_T("<TextBlock Foreground='Red' Text='Open (%s)'/>"), shortcut);
		}
		pCommandBars->GetActions()->FindAction(ID_FILE_OPEN)->SetTooltip(m_bToolBarAccelTips ? str : _T("<TextBlock Foreground='Red' Text='Open'/>"));

		if(m_bToolBarAccelTips)
		{
			pCommandBars->GetShortcutManager()->FindDefaultAccelerator(ID_FILE_SAVE, shortcut);
			str.Format(_T("<TextBlock FontStyle='Italic' TextDecorations='Underline' Text='Save (%s)'/>"), shortcut);
		}
		pCommandBars->GetActions()->FindAction(ID_FILE_SAVE)->SetTooltip(m_bToolBarAccelTips ? str : _T("<TextBlock FontStyle='Italic' TextDecorations='Underline' Text='Save'/>"));

		if(m_bToolBarAccelTips)
		{
			pCommandBars->GetShortcutManager()->FindDefaultAccelerator(ID_EDIT_CUT, shortcut);
			str.Format(_T("Cut (%s)"), shortcut);
		}
		pCommandBars->GetActions()->FindAction(ID_EDIT_CUT)->SetTooltip(
			CreateMarkupTool(m_bToolBarAccelTips ? str : _T("Cut"), _T("Cut the selection and <Underline>put</Underline> it on the Clipboard")));
		
		if(m_bToolBarAccelTips)
		{
			pCommandBars->GetShortcutManager()->FindDefaultAccelerator(ID_EDIT_COPY, shortcut);
			str.Format(_T("Copy (%s)"), shortcut);
		}
		pCommandBars->GetActions()->FindAction(ID_EDIT_COPY)->SetTooltip(
			CreateMarkupTool(m_bToolBarAccelTips ? str : _T("Copy"), _T("Copy the selection and put it on the Clipboard")));
		
		if(m_bToolBarAccelTips)
		{
			pCommandBars->GetShortcutManager()->FindDefaultAccelerator(ID_EDIT_PASTE, shortcut);
			str.Format(_T("Paste (%s)"), shortcut);
		}
		pCommandBars->GetActions()->FindAction(ID_EDIT_PASTE)->SetTooltip(
			CreateMarkupTool(m_bToolBarAccelTips ? str : _T("Paste"), _T("Insert Clipboard contents")));
	
		if(m_bToolBarAccelTips)
		{
			pCommandBars->GetShortcutManager()->FindDefaultAccelerator(ID_FILE_PRINT, shortcut);
			str.Format(_T("Print (%s)"), shortcut);
		}

		pCommandBars->GetActions()->FindAction(ID_FILE_PRINT)->SetTooltip(
			CreateMarkupTool(m_bToolBarAccelTips ? str : _T("Print"), _T("Print the active document")));
	...
	else if (pCommandBars->GetToolTipContext()->GetStyle() == xtpToolTipHTML)
	{
		...
		{
			pCommandBars->GetShortcutManager()->FindDefaultAccelerator(ID_FILE_NEW, shortcut);
			str.Format(_T("<b>New (%s)<b>"), shortcut);
		}
		pCommandBars->GetActions()->FindAction(ID_FILE_NEW)->SetTooltip(m_bToolBarAccelTips ? str : _T("<b>New<b>"));

		if(m_bToolBarAccelTips)
		{
			pCommandBars->GetShortcutManager()->FindDefaultAccelerator(ID_FILE_OPEN, shortcut);
			str.Format(_T("<font color=red>Open (%s)</font>"), shortcut);
		}
		pCommandBars->GetActions()->FindAction(ID_FILE_OPEN)->SetTooltip(m_bToolBarAccelTips ? str : _T("<font color=red>Open</font>"));

		if(m_bToolBarAccelTips)
		{
			pCommandBars->GetShortcutManager()->FindDefaultAccelerator(ID_FILE_SAVE, shortcut);
			str.Format(_T("<i><u>Save (%s)</u></i>"), shortcut);
		}
		pCommandBars->GetActions()->FindAction(ID_FILE_SAVE)->SetTooltip(m_bToolBarAccelTips ? str : _T("<i><u>Save</u></i>"));
		...
		pCommandBars->GetActions()->FindAction(ID_EDIT_CUT)->SetTooltip(
			CreateHTMLTool(m_bToolBarAccelTips ? str : _T("Cut"), _T("Cut the selection and put it on the Clipboard")));
		...
		pCommandBars->GetActions()->FindAction(ID_EDIT_COPY)->SetTooltip(
			CreateHTMLTool(m_bToolBarAccelTips ? str : _T("Copy"), _T("Copy the selection and put it on the Clipboard")));
		...
		pCommandBars->GetActions()->FindAction(ID_EDIT_PASTE)->SetTooltip(
			CreateHTMLTool(m_bToolBarAccelTips ? str : _T("Paste"), _T("Insert Clipboard contents")));
		...
		pCommandBars->GetActions()->FindAction(ID_FILE_PRINT)->SetTooltip(
			CreateHTMLTool(m_bToolBarAccelTips ? str : _T("Print"), _T("Print the active document")));
