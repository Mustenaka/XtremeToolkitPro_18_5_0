...
CRect CExplorerTheme::GetCommandBarBorders(CXTPCommandBar* pBar)
{
	...
	{	
		if (pBar->GetType() == xtpBarTypePopup)
			return XTP_DPI(CRect(6, 1, 1, 1));

		if (pBar->GetType() == xtpBarTypeMenuBar)
			return XTP_DPI(CRect(1, 0, 1, 1));
	}
...
void CExplorerTheme::FillCommandBarEntry(CDC* pDC, CXTPCommandBar* pBar)
{
	...
	if (pBar->GetPosition() == xtpBarPopup && m_bLunaTheme)
	{
		...
		if (pBar->GetType() == xtpBarTypePopup)
		{
			pDC->FillSolidRect(rc.left, rc.top, XTP_DPI_X(6), rc.Height(), m_clrPopupBarBorder);
			pDC->FillSolidRect(rc.left + XTP_DPI_X(1), rc.top + XTP_DPI_Y(1), XTP_DPI_X(1), rc.Height() - XTP_DPI_Y(2), m_clrPopupBarGripperShadow);
		}
	}
	else
	{
