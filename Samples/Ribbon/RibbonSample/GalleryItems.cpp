...
void CGalleryItemStyleSet::Draw(CDC* pDC, CXTPControlGallery* pGallery, CRect rcItem, BOOL bEnabled, BOOL bSelected, BOOL bPressed, BOOL bChecked)
{
	...
	pPaintManager->DrawPopupBarGripper(pDC, rcItem.left - XTP_DPI_X(1), rcItem.top, 
		cx + XTP_DPI_X(1), rcItem.Height());
	...
	{
		CRect rcCheck(rcItem.left + XTP_DPI_X(1), rcItem.top + XTP_DPI_Y(1), 
			rcItem.left + cx - XTP_DPI_X(2), rcItem.bottom - XTP_DPI_Y(1));
...
void CGalleryItemThemeColors::Draw(CDC* pDC, CXTPControlGallery* pGallery, CRect rcItem, BOOL bEnabled, BOOL bSelected, BOOL bPressed, BOOL bChecked)
{
	...
	CRect rcPreview(rcItem.left + XTP_DPI_X(2), rcItem.top + XTP_DPI_Y(2), 
		rcItem.left + XTP_DPI_X(2) + XTP_DPI_X(108), rcItem.bottom - XTP_DPI_Y(2));
...
void CGalleryItemThemeFonts::Draw(CDC* pDC, CXTPControlGallery* pGallery, CRect rcItem, BOOL bEnabled, BOOL bSelected, BOOL bPressed, BOOL bChecked)
{
	...
	CRect rcFontPreviewBody(rcPreview.left + XTP_DPI_X(5), rcPreview.top, 
		rcPreview.right, rcPreview.bottom - XTP_DPI_Y(5));
	CXTPFontDC fnt(pDC, &fntBody);
	pDC->DrawText(_T("A"), rcFontPreviewBody, DT_BOTTOM | DT_SINGLELINE);

	CRect rcFontPreviewBodyText(rcItem.left + XTP_DPI_X(70), rcItem.top + XTP_DPI_Y(17), 
		rcItem.right, rcItem.bottom);
	pDC->DrawText(fontPreview.lpszBody, rcFontPreviewBodyText, DT_TOP | DT_SINGLELINE);

	CRect rcFontPreviewHeadings(rcPreview.left + XTP_DPI_X(20), rcPreview.top, 
		rcPreview.right, rcPreview.bottom - XTP_DPI_Y(5));
	fnt.SetFont(&fntHeadings);
	pDC->DrawText(_T("a"), rcFontPreviewHeadings, DT_BOTTOM | DT_SINGLELINE);

	CRect rcFontPreviewHeadingsText(rcItem.left + XTP_DPI_X(70), rcItem.top + XTP_DPI_Y(37), 
		rcItem.right, rcItem.bottom);
	...
	else
	{
		if (m_bTopMargin)
		{
			pDC->FillSolidRect(rcItem.left, rcItem.top,
				rcItem.Width(), XTP_DPI_Y(1), RGB(197, 197, 197));
		}

		pDC->FillSolidRect(rcItem.left, rcItem.top, 
			XTP_DPI_Y(1), rcItem.Height(), RGB(197, 197, 197));
		pDC->FillSolidRect(rcItem.right - XTP_DPI_X(1), rcItem.top, 
			XTP_DPI_X(1), rcItem.Height(), RGB(197, 197, 197));
