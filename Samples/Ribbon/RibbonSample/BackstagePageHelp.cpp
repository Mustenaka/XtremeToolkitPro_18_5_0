...
BOOL CBackstagePageHelp::OnInitDialog()
{
	...
	m_lblTools.SetFont(&m_xtpFontCaption);
	m_lblTools.SetTextColor(RGB(94, 94, 94));

	m_lblSupport.SetFont(&m_xtpFontCaption);
	m_lblSupport.SetTextColor(RGB(94, 94, 94));

	m_lblAbout.SetFont(&m_xtpFontCaption);
...
void CBackstagePageHelp::SetTheme(XTPControlTheme nTheme)
{
	CXTPRibbonBackstagePage::SetTheme(nTheme);
	
	m_lblSupport.SetTheme(nTheme);
	m_lblTools.SetTheme(nTheme);
	m_lblAbout.SetTheme(nTheme);
	...
	m_btnHelp.SetTheme(nTheme);
	m_btnGettingStarted.SetTheme(nTheme);
	m_btnContactUs.SetTheme(nTheme);
	m_btnOptions.SetTheme(nTheme);
	m_btnCheckForUpdates.SetTheme(nTheme);
}
