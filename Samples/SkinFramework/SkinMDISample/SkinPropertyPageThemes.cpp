...
void CSkinPropertyPageThemes::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_APPLYMENUS, m_bApplyMenus);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(CSkinPropertyPageThemes, CPropertyPage)
	...
	ON_BN_CLICKED(IDC_CHECK_APPLYMENUS, OnCheckApplyMenus)
...
void CSkinPropertyPageThemes::OnCheckApplyMenus()
{
	UpdateData();
	...
	if (m_bApplyMenus)
		XTPSkinManager()->SetApplyOptions(dwApplyOptions | xtpSkinApplyMenus);
	else
		XTPSkinManager()->SetApplyOptions(dwApplyOptions & ~xtpSkinApplyMenus);
