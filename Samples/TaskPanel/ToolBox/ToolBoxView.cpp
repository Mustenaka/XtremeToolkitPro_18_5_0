...
int CToolBoxView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_wndPropertySheet.GetPaintManager()->SetAppearance(xtpTabAppearanceExcel2003);
	m_wndPropertySheet.ModifyStyleEx(0, WS_EX_CONTROLPARENT);
#else

	if (!m_wndPropertySheet.Create(WS_CHILD|WS_VISIBLE|WS_CLIPSIBLINGS|WS_CLIPCHILDREN|WS_TABSTOP, CRect(0, 0, 0, 0), this, 0))
		return -1;
	...
	CXTPTaskPanelGroup* pFolder = m_wndPropertySheet.AddGroup(0);
	pFolder->SetCaption(_T("Look And Behaviour"));
	pFolder->AddControlItem(m_wndPageLookAndBehaviour);

	pFolder = m_wndPropertySheet.AddGroup(0);
	pFolder->SetCaption(_T("Drag And Drop"));
	pFolder->AddControlItem(m_wndPageDragAndDrop);


	m_wndPropertySheet.SetTheme(xtpTaskPanelThemeVisualStudio2003);
	m_wndPropertySheet.Reposition(FALSE);
