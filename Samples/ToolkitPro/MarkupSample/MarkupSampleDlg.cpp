...
void CMarkupSampleDlg::OnHyperlinkClick(CXTPMarkupObject* pSender, CXTPMarkupRoutedEventArgs* pArgs)
{
	if (pSender->IsKindOf(MARKUP_TYPE(CXTPMarkupHyperlink)))
	{
		...
		if (pTag)
		{
			...
			CString str;
			str.Format(_T("Hyperlink Clicked. Tag = %ls"), (LPCWSTR)*pTag);
			AfxMessageBox(str);
		}
		else
			AfxMessageBox(_T("Hyperlink Clicked"));
...
BOOL CMarkupSampleDlg::OnInitDialog()
{
	...
	m_wndStatic.SetMarkupText(
		_T("<Border xmlns='http://schemas.microsoft.com/winfx/2006/xaml/presentation' xmlns:x='http://schemas.microsoft.com/winfx/2006/xaml' ")
		_T("   Padding='4' BorderThickness='1' BorderBrush='#767676' Background='#e4ecf7'>")
	...
	m_wndTree.AddItem(_T("<TextBlock><Bold>Inbox</Bold> <Hyperlink Tag='tree1'><Run Foreground='Blue' Text='[1]' /></Hyperlink></TextBlock>"), -1, -1, hItemRoot);
	m_wndTree.AddItem(_T("<TextBlock><Bold>Junk E-mail</Bold> <Run Foreground='Green' Text='[3]'/></TextBlock>"), -1, -1, hItemRoot);
	m_wndTree.AddItem(_T("<TextBlock><Bold>Node with a long text that requires scrolling</Bold></TextBlock>"), -1, -1, hItemRoot);
	...
	m_wndTabControl.SetAppearance(xtpTabAppearanceVisualStudio2003);
...
CXTPPopupControl* pPopup = new CXTPPopupControl();
	...
	LPCTSTR lpszMarkup = 
	_T("<Grid xmlns='http://schemas.microsoft.com/winfx/2006/xaml/presentation' xmlns:x='http://schemas.microsoft.com/winfx/2006/xaml' ")
	_T("    VerticalAlignment='Center' HorizontalAlignment='Center'>")
...
{
	// It is important to NOT PASS utf8 XAML as Unicode for Unicode builds,
	// as MSXML won't understand mixing claimed UTF-8 XML encoding
	// and UTF-16 charset of the source data. Same is true for any other encoding.
	m_wndStatic.SetMarkupText(
...
void CMarkupSampleDlg::OnTabControlSelChanged(NMHDR* pHeader, LRESULT* pResult)
{
	...
	{
		AddListItem(_T("<Border xmlns='http://schemas.microsoft.com/winfx/2006/xaml/presentation' xmlns:x='http://schemas.microsoft.com/winfx/2006/xaml' ")
			_T("BorderThickness='2' BorderBrush='DodgerBlue' Margin='2' Padding='3'><StackPanel Orientation='Horizontal'>")
		...
		AddListItem(_T("<Border xmlns='http://schemas.microsoft.com/winfx/2006/xaml/presentation' xmlns:x='http://schemas.microsoft.com/winfx/2006/xaml' ")
			_T("BorderThickness='2' BorderBrush='Orange' Margin='2' Padding='3'><StackPanel Orientation='Horizontal'>")
			_T("<Image Source='10'/>")
			_T("<Grid><Grid.ColumnDefinitions><ColumnDefinition Width='Auto'/><ColumnDefinition Width='*'/></Grid.ColumnDefinitions>")
			_T("<Grid.RowDefinitions><RowDefinition/><RowDefinition/></Grid.RowDefinitions>")
			_T("<TextBlock TextAlignment='Right' FontWeight='Bold' Foreground='Navy' Text='Description:'/>")
