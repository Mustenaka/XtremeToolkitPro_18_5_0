...
void CTabManagerPage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_MULTIROWOPTIMIZATION, m_bMultiRowTabsOptimization);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(CTabManagerPage, CXTPPropertyPage)
	...
	ON_BN_CLICKED(IDC_CHECK_MULTIROWOPTIMIZATION, OnCheckOptimization)
...
void CTabManagerPage::Update()
{
	...
	m_bMultiRowTabsOptimization = GetPaintManager()->m_bMultiRowTabsOptimization;
...
BOOL CTabManagerPage::SetResourceHandle(XTPTabColorStyle colorStyle)
{
	BOOL bResult = TRUE;
	...
	case xtpTabColorOffice2013:
		bResult = XTPThemeDLL()->SetHandle(xtpIniOffice2013Word);
		break;
	case xtpTabColorVisualStudio2015:
		bResult = XTPThemeDLL()->SetHandle(xtpIniVisualStudio2015Light);
		break;
	default:
		bResult = XTPThemeDLL()->SetHandle(xtpIniOffice2007Blue);
		break;
	}
...
void CTabManagerPage::OnSelchangeComboColor()
{
	...
	XTPTabColorStyle colorStyle = (XTPTabColorStyle)m_comboColor.GetItemData(m_nColor);
	
	SetResourceHandle(colorStyle);

	GetPaintManager()->SetColor(colorStyle);
...
void CTabManagerPage::OnSelchangeComboAppearance()
{
	...
	CXTPTabPaintManagerTheme* theme = NULL;
	theme = GetPaintManager()->SetAppearance((XTPTabAppearanceStyle)m_nAppearance);
	
	XTPTabColorStyle colorStyle = theme->GetDefaultColorSet();
	SetResourceHandle(colorStyle);
	GetPaintManager()->SetColor(colorStyle);
...
void CTabManagerPage::UpdateControls()
{
	...
	colors[] =
	{
		...
		xtpTabColorVisualStudio2012, _T("xtpTabColorVisualStudio2012"),
		xtpTabColorOffice2013, _T("xtpTabColorOffice2013"),
		xtpTabColorVisualStudio2015, _T("xtpTabColorVisualStudio2015")
	};
	...
	GetDlgItem(IDC_CHECK_MULTIROWOPTIMIZATION)->EnableWindow(m_nLayout == xtpTabLayoutMultiRow);
}
...
void CTabManagerPage::OnCheckOptimization()
{
	UpdateData();
	GetPaintManager()->m_bMultiRowTabsOptimization = m_bMultiRowTabsOptimization;	
	Reposition();
}
