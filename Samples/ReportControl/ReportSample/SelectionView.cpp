...
void CSelectionView::OnReportValueChanged(NMHDR *pNotifyStruct, LRESULT *pResult)
{
	...
	CXTPReportRecord  *pFilterRecord = wndReport.GetHeaderRecords()->GetAt(0);
	CXTPReportRecordItemText *pFilterItem = NULL;
	CXTPReportRecordItemText *pItem = NULL;
	int nColumn = 0;
	int nRecord = 0;
	BOOL bVisibility = TRUE;

	for (nRecord = 0; nRecord < wndReport.GetRecords()->GetCount(); nRecord++)
	{
		pRecord = wndReport.GetRecords()->GetAt(nRecord);
		if (pRecord == NULL)
			continue;
		
		bVisibility = TRUE;

		for(nColumn = 0; nColumn < pFilterRecord->GetItemCount(); nColumn++)
		{
			pFilterItem = reinterpret_cast<CXTPReportRecordItemText*>(pFilterRecord->GetItem(nColumn));
			if (pFilterItem == NULL)
				continue;

			CString sFilter = pFilterItem->GetValue();
			if (sFilter == _T(""))
				continue;

			pItem = reinterpret_cast<CXTPReportRecordItemText*>(pRecord->GetItem(nColumn));
			if (pItem == NULL)
				continue;

			if (sFilter != pItem->GetValue().Left(sFilter.GetLength()))
			{
				bVisibility = FALSE;
				break;
			}
		...
		pRecord->SetVisible(bVisibility);
	}
