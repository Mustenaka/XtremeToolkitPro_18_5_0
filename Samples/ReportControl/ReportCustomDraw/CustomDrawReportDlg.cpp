...
BOOL CCustomDrawReportDlg::OnInitDialog()
{
	...
	{
		cx = cy = 16;
		VERIFY(bitmap.LoadBitmap(IDB_CONTROLS_16));
	}
	else if (120 <= dpi && dpi < 144)
	{
		cx = cy = 20;
		VERIFY(bitmap.LoadBitmap(IDB_CONTROLS_20));
	}
	else if (144 <= dpi && dpi < 192)
	{
		cx = cy = 24;
		VERIFY(bitmap.LoadBitmap(IDB_CONTROLS_24));
	}
	else
	{
		cx = cy = 32;
		VERIFY(bitmap.LoadBitmap(IDB_CONTROLS_32));
	}

	//
	// create the image lists used by the report control.
	//
	if (!m_ilControls.Create(cx, cy, ILC_COLOR24 | ILC_MASK, 0, 1))
		return -1;
...
int CCustomDrawReportDlg::GetRowHeight(CDC* /*pDC*/, CXTPReportRow* pRow)
{
	...
	if (!pRow->IsGroupRow())
	{
		return unControls * XTP_DPI_X(15) + (m_wndReportCtrl.GetPaintManager()->IsGridVisible(FALSE) ? XTP_DPI_X(1) : XTP_DPI_X(0));
	}

	if (m_wndReportCtrl.GetPaintManager()->m_bShadeGroupHeadings)
		return unControls * XTP_DPI_X(15) + XTP_DPI_X(6);

	return unControls * XTP_DPI_X(15) + XTP_DPI_X(16);
}
...
BOOL CCustomDrawReportDlg::DrawItem(XTP_REPORTRECORDITEM_DRAWARGS* pDrawArgs, XTP_REPORTRECORDITEM_METRICS* /*pMetrics*/)
{
	...
	rc.left += XTP_DPI_X(5);
    rc.right = rc.left + XTP_DPI_X(14);
    rc.bottom = rc.top + XTP_DPI_Y(14);
	for(unsigned int i = 0; i < GetControlsNumber(pDrawArgs->pItem); i++)
	{
		...
		TextOut(pDrawArgs->pDC->GetSafeHdc(), rc.left + XTP_DPI_X(20), rc.top, strText, strText.GetLength());
		rc.top += XTP_DPI_Y(15);
        rc.bottom += XTP_DPI_Y(15);
	}
	...
	if(pMsg->message == WM_LBUTTONDOWN)
	{
		...
		if(pRow)
		{
			...
			if(pItem)
			{
				...
				rc.right = rc.left + XTP_DPI_X(14) + XTP_DPI_X(25);
				rc.bottom = rc.top + XTP_DPI_Y(14);
				for(unsigned int i = 0; i < unControls; i++)
				{
					...
					rc.OffsetRect(XTP_DPI_X(0), XTP_DPI_Y(15));
				}
			}
		}
	}
