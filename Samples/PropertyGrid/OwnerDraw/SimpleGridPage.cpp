...
void CSimpleGridPage::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	CXTPFontDC font(&dc, pItem->IsCategory()? &pMetrics->m_xtpFontBold: &pMetrics->m_xtpFontNormal);
	...
	COLORREF clrFore = GetSysColor(COLOR_BTNTEXT);
	{
		if (!pItem->IsCategory())
		{
			...
			CRect rcValue(rc);
			rcValue.left = rcCaption.right + XTP_DPI_X(4);
			rcValue.bottom -= XTP_DPI_Y(1);
		...
		if (bFocused)
		{
			...
			dc.FillSolidRect(CRect(XTP_DPI_X(XTP_PGI_EXPAND_BORDER), rcCaption.top, rcCaption.right, rcCaption.bottom), ::GetSysColor(COLOR_HIGHLIGHT));
		}
		else if (bSelected)
		{
		...
		dc.MoveTo(XTP_DPI_X(XTP_PGI_EXPAND_BORDER), rc.bottom - 1); dc.LineTo(rc.right, rc.bottom - 1);
	...
	if (pItem->HasChilds())
	{
		...
		CRect rcSign(CPoint(XTP_DPI_X(XTP_PGI_EXPAND_BORDER) / 2 - XTP_DPI_X(5), rc.CenterPoint().y - XTP_DPI_Y(4)), 
			XTP_DPI(CSize(9, 9)));

		if (pItem->GetIndent() > 1)
			rcSign.OffsetRect((pItem->GetIndent() - 1) * XTP_DPI_X(XTP_PGI_EXPAND_BORDER), 0);


		CPoint pt = rcSign.CenterPoint();

		CXTPBrushDC brush (dc,  clrWindow);
		dc.Draw3dRect(rcSign, clrFore, clrFore);

		XTPPrimitiveSymbol nSymbol = (pItem->IsExpanded()
			? xtpPrimitiveSymbolMinus
			: xtpPrimitiveSymbolPlus);
		XTPPrimitiveDrawer()->DrawSymbol(&dc, nSymbol, rcSign, clrFore);
