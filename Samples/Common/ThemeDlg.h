...
struct THEME_COLOR
{
	...
	THEME_COLOR(int t, int c, int a, int b)
		: nTheme(t)
		, nColor(c)
		, nAccent(a)
		, nBack(b)
...
{
public:
	CThemeDlg(CWnd* pParent = NULL) : CDialog(CThemeDlg::IDD, pParent)
	{
		m_bPreview = FALSE;
		m_dwExclude = NULL;
	...
	virtual void SetTheme(int nTheme, int nColor = xtpThemeColorUndef, int nAccent = xtpThemeAccentUndef, int nBack = xtpThemeRibbonBackUndef, BOOL bNotify = FALSE)
	{
	...
	virtual void AddCustomTheme(int nID, LPCTSTR lpszTheme, BOOL bSelected = FALSE, int nInsert = -1)
	{
		if (!::IsWindow(m_hWnd))
			return;
...
protected:
	...
	virtual void BuildThemeList()
	{
		...
		for (int i = 0; i < _countof(g_szThemes); ++i)
		{
			...
			switch (i)
			{
			case xtpThemeDlgOffice2000:       { if (m_dwExclude & XTP_EXCLUDE_THEME_OFFICE2000)       continue; } break;
			case xtpThemeDlgOfficeXP:         { if (m_dwExclude & XTP_EXCLUDE_THEME_OFFICEXP)         continue; } break;
			case xtpThemeDlgOffice2003:       { if (m_dwExclude & XTP_EXCLUDE_THEME_OFFICE2003)       continue; } break;
			case xtpThemeDlgOffice2007:       { if (m_dwExclude & XTP_EXCLUDE_THEME_OFFICE2007)       continue; } break;
			case xtpThemeDlgOffice2010:       { if (m_dwExclude & XTP_EXCLUDE_THEME_OFFICE2010)       continue; } break;
			case xtpThemeDlgOffice2013:       { if (m_dwExclude & XTP_EXCLUDE_THEME_OFFICE2013)       continue; } break;
			case xtpThemeDlgOffice2016:       { if (m_dwExclude & XTP_EXCLUDE_THEME_OFFICE2016)       continue; } break;
			case xtpThemeDlgVisualStudio6:    { if (m_dwExclude & XTP_EXCLUDE_THEME_VISUALSTUDIO6)    continue; } break;
			case xtpThemeDlgVisualStudio2005: { if (m_dwExclude & XTP_EXCLUDE_THEME_VISUALSTUDIO2005) continue; } break;
			case xtpThemeDlgVisualStudio2008: { if (m_dwExclude & XTP_EXCLUDE_THEME_VISUALSTUDIO2008) continue; } break;
			case xtpThemeDlgVisualStudio2010: { if (m_dwExclude & XTP_EXCLUDE_THEME_VISUALSTUDIO2010) continue; } break;
			case xtpThemeDlgVisualStudio2012: { if (m_dwExclude & XTP_EXCLUDE_THEME_VISUALSTUDIO2012) continue; } break;
			case xtpThemeDlgVisualStudio2015: { if (m_dwExclude & XTP_EXCLUDE_THEME_VISUALSTUDIO2015) continue; } break;
			case xtpThemeDlgNativeWindows:    { if (m_dwExclude & XTP_EXCLUDE_THEME_NATIVEWINDOWS)    continue; } break;
			case xtpThemeDlgCustom:           { if (m_dwExclude & XTP_EXCLUDE_THEME_CUSTOM)           continue; } break;
			}
	...
	virtual void SendNotify(const THEME_COLOR& tc)
	{
		...
		{
			m_pParentWnd->SendMessage(WM_XTP_THEMECHANGED, MAKEWPARAM(tc.nTheme, tc.nColor),
				MAKELPARAM(tc.nAccent, tc.nBack));
		}
	}
