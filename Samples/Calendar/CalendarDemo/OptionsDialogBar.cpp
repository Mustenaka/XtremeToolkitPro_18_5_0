...
BEGIN_MESSAGE_MAP(COptionsDialogBar, CDialogBar)
	...
	ON_CBN_SELCHANGE(IDC_MIN_SCALE_TIME_CB, OnCbnSelchangeMinMaxScaleTimeCb)
	ON_CBN_SELCHANGE(IDC_MAX_SCALE_TIME_CB, OnCbnSelchangeMinMaxScaleTimeCb)
	...
	ON_BN_CLICKED(IDC_CHK_MULTI_COLUMN_MODE, OnMultiColumnMode)
...
void COptionsDialogBar::OnInitDialog() 
{
	...
	{	
		...
		VERIFY( m_ctrlMinScaleTimeCb.SubclassDlgItem(IDC_MIN_SCALE_TIME_CB, this));
		VERIFY( m_ctrlMaxScaleTimeCb.SubclassDlgItem(IDC_MAX_SCALE_TIME_CB, this));
		...
		m_arOptCtrlIDsCommon.Add(IDC_MIN_SCALE_TIME_CB);
		m_arOptCtrlIDsCommon.Add(IDC_START_TIME_TEXT2);
		m_arOptCtrlIDsCommon.Add(IDC_MAX_SCALE_TIME_CB);
...
void COptionsDialogBar::OnMultiColumnMode()
{
	// IDC_CHK_MULTI_COLUMN_MODE
	BOOL b = ((CButton*) GetDlgItem(IDC_CHK_MULTI_COLUMN_MODE))->GetCheck();
	CXTPCalendarControl* pCalendar = GetCalendarCtrl();
	if (pCalendar)
	{
