...
void CDataProvidersOptions::OnOK() 
{
	...
	if(m_ctrlUseMemDP.GetCheck())		
	{
		...
		{
			CString strMsg;
			strMsg.Format(IDS_MSG_EXT, cstrMemExt.operator LPCTSTR());
		...
		//*******************
		m_strConnectionString.Format(_T("Provider=XML;Data Source='%s';"), strDataFile.operator LPCTSTR());
	...
	else if(m_ctrlUseDBDP.GetCheck())
	{
		...
		{
			CString strMsg;
			strMsg.Format(IDS_MSG_EXT, cstrDbExt.operator LPCTSTR());
			if (AfxMessageBox(strMsg, MB_YESNO) == IDYES)
			{
		...
		//*******************
		m_strConnectionString.Format(_T("Provider=Microsoft.Jet.OLEDB.4.0;Data Source='%s';"), 
			strDataFile.operator LPCTSTR());
