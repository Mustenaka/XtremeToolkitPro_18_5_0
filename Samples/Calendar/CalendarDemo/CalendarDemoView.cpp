...
_T("<StackPanel Margin='2, 2, 2, 2' VerticalAlignment='Top' HorizontalAlignment='Center'>")
...
BEGIN_MESSAGE_MAP(CCalendarDemoView, CXTPCalendarControlView)
	...
	ON_COMMAND(ID_CALENDAR_GOTO_TODAY, OnGoToToday)	
...
//	ON_COMMAND(ID_CALENDAR_LANGUAGE_DEFAULT,  OnCalendarLangDefault)
	...
	ON_COMMAND(ID_CALENDAR_VIEW_SCHEDULE, OnCalendarTimeline)
	ON_UPDATE_COMMAND_UI(ID_CALENDAR_VIEW_SCHEDULE, OnUpdateCalendarTimeline)

	ON_COMMAND(ID_CATEGORIES_ADDTEST, OnAddTestCategory)
	ON_UPDATE_COMMAND_UI(ID_CATEGORIES_ADDTEST, OnUpdateAddTestCategory)

	ON_COMMAND_RANGE(ID_CATEGORIES_ALLCATEGORIES, ID_CATEGORIES_ALLCATEGORIESANDLABELS, OnCategories)

	ON_COMMAND(ID_CALENDAR_MEETING_OPEN, OnCalendarMeetingOpen)
	ON_COMMAND(ID_CALENDAR_MEETING_DELETE, OnCalendarMeetingDelete)
	ON_COMMAND(ID_CALENDAR_MEETING_PRIVATE, OnCalendarMeetingPrivate)
	ON_COMMAND(ID_CALENDAR_MEETING_MEETING, OnCalendarMeetingMeeting)
	ON_COMMAND(ID_CALENDAR_MEETING_REMINDER, OnCalendarMeetingReminder)

	ON_UPDATE_COMMAND_UI(ID_CALENDAR_MEETING_PRIVATE, OnUpdateCalendarMeetingPrivate)
	ON_UPDATE_COMMAND_UI(ID_CALENDAR_MEETING_MEETING, OnUpdateCalendarMeetingMeeting)
	ON_UPDATE_COMMAND_UI(ID_CALENDAR_MEETING_REMINDER, OnUpdateCalendarMeetingReminder)
...
CCalendarDemoView::CCalendarDemoView()
{
	...
	m_Sink.Advise(ptrCalendarConn, XTP_NC_CALENDAR_SELECTION_CHANGED, &CCalendarDemoView::OnEventSelChanged);
...
int CCalendarDemoView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_wndHScrollBar.Create(WS_CHILD | WS_VISIBLE | SBS_HORZ, CRect(0, 0, 0, 0), this, 101);
	m_wndHScrollBar.SetScrollBarStyle(xtpScrollStyleOffice2007Dark);
	SetScrollBarCtrl(&m_wndHScrollBar, SB_HORZ);
#endif
	return 0;
}
...
void CCalendarDemoView::OnCalendarViewWeek() 
{
	...
	if (!GetCalendarCtrl().m_bTimelineMode)
	{
		...
		{
			GetCalendarCtrl().m_bMultiColumnWeekMode = TRUE;
...
void CCalendarDemoView::OnInitialUpdate() 
{
	...
	SetTheme(xtpCalendarThemeOffice2013);
	OnCalendarViewWorkWeek();
...
{   
	...
	//GetCalendarCtrl().GetDayView()->SetScaleMinTime(COleDateTime(2013, 01, 23, 9, 00, 00));
	//GetCalendarCtrl().GetDayView()->SetScaleMinTime(COleDateTime(2013, 01, 23, 18, 00, 00));
}

void CCalendarDemoView::OnEvent_Calendar(XTP_NOTIFY_CODE idEvent, WPARAM wParam, LPARAM lParam)
{
	...
	if (XTP_NC_CALENDARKEYDOWN == idEvent)
	{
	...
	else if (XTP_NC_CALENDARCONTEXTMENU == idEvent)
	{
		if (bHitTest)
		{
			CPoint ptMenu = ptHit;
	...
	else if (XTP_NC_CALENDARLBUTTONDBLCLICK == idEvent)
	{
	...
	if (GetDPType(pData) == xtpCalendarDataProviderMemory) 
	{
		...
		if (!pData->Save()) 
		{
			...
			strMsg.Format(_T("%s cannot create file: \n%s"), 
				strDPname.operator LPCTSTR(), strFileName.operator LPCTSTR());
...
void CCalendarDemoView::OnGoToToday()
{
	CXTPCalendarView* pView = GetCalendarCtrl().GetActiveView();	
	if (pView)
	{
		pView->ResetSelection();
		COleDateTime dtThisDay = COleDateTime::GetCurrentTime();						
		pView->SelectDay(dtThisDay);
	}
}
...
void CCalendarDemoView::OnEvent_IsEditActionDisabled(XTP_NOTIFY_CODE idEvent, 
													WPARAM wParam, LPARAM lParam)
{   	
	if (XTP_NC_CALENDAR_IS_EVENT_EDIT_OPERATION_DISABLED != idEvent)
	{
		ASSERT(FALSE);
		return;
	}
...
void CCalendarDemoView::OnEvent_BeforeEditAction(XTP_NOTIFY_CODE idEvent, 
													WPARAM wParam, LPARAM lParam)
{   	
	if (XTP_NC_CALENDAR_BEFORE_EVENT_EDIT_OPERATION != idEvent)
	{
		ASSERT(FALSE);
		return;
	}
	...
	if (pOPparams->eOperation == xtpCalendarEO_DeleteEvent)
	{
		...
		{
			CString strMsg;
			strMsg.Format(_T("Would you like to delete event: \"%s\" ?"), 
				pEvent->GetSubject().operator LPCTSTR());
...
void CCalendarDemoView::OnEvent_CalendarData(XTP_NOTIFY_CODE idEvent, WPARAM wParam, LPARAM lParam)
{
	UNREFERENCED_PARAMETER(lParam);
	...
	if (idEvent == XTP_NC_CALENDAREVENTWASADDED || idEvent == XTP_NC_CALENDAREVENTWASDELETED ||
		idEvent == XTP_NC_CALENDAREVENTWASCHANGED)
	{
		//EVENT:
		...
		if (pEvent)
		{
			...
			TRACE(_T("%s (ID=%d): %s-%s, '%s' \n"), strActionText.operator LPCTSTR(),
				pEvent->GetEventID(), dtStart.Format().operator LPCTSTR(), 
				dtEnd.Format().operator LPCTSTR(), strSubject.operator LPCTSTR());
		} 
		else
		{
			TRACE(_T("%s (pEvent=%x) \n"), strActionText.operator LPCTSTR(), wParam);
		}
	}
	else
	{
		...
		if (ptrPattern)
		{
			...
			TRACE(_T("%s (ID=%d): pattern starts at %s, '%s' \n"), strActionText.operator LPCTSTR(),
					wParam, dtStartPatt.Format().operator LPCTSTR(), strSubject.operator LPCTSTR());
		} 
		else
		{
			TRACE(_T("%s (ID=%d) \n"), strActionText.operator LPCTSTR(), wParam);
...
//void CCalendarDemoView::OnReminders(XTP_NOTIFY_CODE idEvent, WPARAM wParam , LPARAM lParam)
//{
//	UNREFERENCED_PARAMETER(lParam);
//	
//	if (idEvent != XTP_NC_CALENDAR_ON_REMINDERS)
//{
//		ASSERT(FALSE);
//		return;
//	}
...
void CCalendarDemoView::OnBeforeDrawThemeObject(XTP_NOTIFY_CODE idEvent, WPARAM wParam , LPARAM lParam)
{
	ASSERT(idEvent == XTP_NC_CALENDAR_BEFORE_DRAW_THEMEOBJECT);
...
void CCalendarDemoView::OnEvent_GetItemText(XTP_NOTIFY_CODE idEvent, WPARAM wParam, LPARAM lParam)
{
	UNREFERENCED_PARAMETER(lParam);
	XTP_CALENDAR_GETITEMTEXT_PARAMS* pParams = (XTP_CALENDAR_GETITEMTEXT_PARAMS*)wParam ;
	
	if (XTP_NC_CALENDAR_GETITEMTEXT != idEvent || !pParams)
		return;
		...
		if ( ((CXTPCalendarControl*)this)->GetActiveView()->HitTest(point, &hitTest))
		{
			if (hitTest.pViewEvent) 
			{
				...
				strToolText.Format(_T("[ID = 0x%08X] (%s - %s) %s"),
					...
					hitTest.pViewEvent->FormatEventTime(hitTest.pViewEvent->GetEvent()->GetStartTime()).operator LPCTSTR(),
					hitTest.pViewEvent->FormatEventTime(hitTest.pViewEvent->GetEvent()->GetEndTime()).operator LPCTSTR(),
					hitTest.pViewEvent->GetEvent()->GetSubject().operator LPCTSTR()
					);
...
void CCalendarDemoView::OnEvent_PrePopulate(XTP_NOTIFY_CODE idEvent, 
											 WPARAM wParam, LPARAM lParam)
{   	
	UNREFERENCED_PARAMETER(wParam);

	if (XTP_NC_CALENDAR_PREPOPULATE != idEvent)
	{
		ASSERT(FALSE);
		return;
	}
...
void CCalendarDemoView::OnEventSelChanged(XTP_NOTIFY_CODE idEvent, WPARAM wParam, LPARAM /*lParam*/)
{   	
#ifdef _XTP_INCLUDE_COMMANDBARS

	ASSERT(XTP_NC_CALENDAR_SELECTION_CHANGED == idEvent);

	if (XTP_NC_CALENDAR_SELECTION_CHANGED != idEvent)
		return;

	if (wParam != xtpCalendarSelectionDays) 
		return;
	...
	CXTPRibbonBar* pRibbonBar =  pMainFrame->m_pRibbonBar;
	if (!pRibbonBar)
		return;
	
	CXTPRibbonTab* pTabMeeting = pRibbonBar->FindTab(ID_TAB_MEETING);
	
	if (pView->IsSingleEventSelected())
	{
		CXTPCalendarViewEvent* pViewEvent = pView->GetLastSelectedViewEvent();
		...
		if (!pTabMeeting)
		{
			pTabMeeting = pRibbonBar->AddTab(ID_TAB_MEETING);
			
			if (!pTabMeeting)
				return;

			pTabMeeting->SetContextTab(xtpRibbonTabContextColorGreen, _T("Calendar Tools"));

			CXTPRibbonGroup* pGroupActions = pTabMeeting->AddGroup(_T("Actions"));
			if (pGroupActions)
			{
				pGroupActions->Add(xtpControlButton, ID_CALENDAR_MEETING_OPEN);
				pGroupActions->Add(xtpControlButton, ID_CALENDAR_MEETING_DELETE);
			}

			CXTPRibbonGroup* pGroupTags = pTabMeeting->AddGroup(_T("Tags"));
			if (pGroupTags)
			{
				CXTPControlPopup* pPopup = (CXTPControlPopup*)pGroupTags->Add(xtpControlPopup, ID_CALENDAR_MEETING_CATEGORIZE);

				CMenu menu;
				VERIFY(menu.LoadMenu(IDR_MENU_CATEGORIES));
				pPopup->SetCommandBar(menu.GetSubMenu(0));

				pGroupTags->Add(xtpControlButton, ID_CALENDAR_MEETING_REMINDER)->SetStyle(xtpButtonIconAndCaption);
				pGroupTags->Add(xtpControlButton, ID_CALENDAR_MEETING_PRIVATE)->SetStyle(xtpButtonIconAndCaption);
				pGroupTags->Add(xtpControlButton, ID_CALENDAR_MEETING_MEETING)->SetStyle(xtpButtonIconAndCaption);
			}
		
			if (pTabMeeting)
			{
				pRibbonBar->SetCurSel(pTabMeeting->GetIndex());
			}
		}
	}
	else
	{
		if (pTabMeeting)
		{
			pRibbonBar->RemoveTab(pTabMeeting->GetIndex());
		}
	}
...
void CCalendarDemoView::SetTheme(XTPCalendarTheme nCalendarTheme)
{
	CXTPCalendarControl::CUpdateContext updateContext(&GetCalendarCtrl(), xtpCalendarUpdateAll);

	CXTPCalendarControl& wndCalendar = GetCalendarCtrl();
	wndCalendar.SetPaintTheme(nCalendarTheme);
	wndCalendar.SetBorders(XTP_DPI_X(1), 0, 0, 0);

	CXTPCalendarCaptionBarControl& wndCaptionBar = GetCalendarCaptionBarCtrl();
	CXTPCalendarCaptionBarTheme* pTheme = wndCaptionBar.GetTheme();
	if (pTheme)
	{
		pTheme->SetSwitchViewButtons(!(nCalendarTheme >= xtpCalendarThemeOffice2013));
		_ResizeControls();
	}

	if (nCalendarTheme == xtpCalendarThemeOffice2013 || 
		nCalendarTheme == xtpCalendarThemeVisualStudio2015)
	{
		wndCaptionBar.SetBorders(XTP_DPI_X(1), 0, 0, 0);
	}
	else
	{
		wndCaptionBar.SetBorders(XTP_DPI_X(1), XTP_DPI_Y(1), 0, 0);
	}
	...
	if (nCalendarTheme == xtpCalendarThemeResource)
	{
		...
		if (pTheme2007)
		{
			CXTPImageManager* pIM = pTheme2007->GetCustomIconsList();
			if (pIM)
			{
	...
	{
		m_wndVScrollBar.SetScrollBarStyle(xtpScrollStyleDefault);
		m_wndHScrollBar.SetScrollBarStyle(xtpScrollStyleDefault);
	}
#endif
}
...
void CCalendarDemoView::OnAddTestCategory()
{
	if (m_bTestCategory)
		return;
	...
	if (pData)
	{
		COLORREF clrTeal  = RGB(117,202,177);
		COLORREF clrOlive = RGB(171,187,141);

		// add a new label to the calendar event label list.
		int nIndex = XTP_SAFE_GET2(pData, GetLabelList(), GetCount(), 0);
		pData->AddEventLabel(nIndex, _T("Olive Label (Test)"), clrOlive);

		// add a new category to the calendar event category list.
		UINT uID = XTP_SAFE_GET2(pData, GetEventCategories(), GetCount(), 1) + 1;
		pData->AddEventCategory(uID, _T("Teal Category (Test)"), CXTPDrawHelpers::DarkenColorLum(clrTeal, 0.60), clrTeal);

		m_bTestCategory = TRUE;
	}
}

void CCalendarDemoView::OnUpdateAddTestCategory(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(m_bTestCategory);
}
...
#else
	CXTPCalendarControlView::OnFilePrintPreview();
#endif
}

CXTPCalendarEvent* CCalendarDemoView::GetSelEvent()
{
	return XTP_SAFE_GET2(GetCalendarCtrl().GetActiveView(),
		GetLastSelectedViewEvent(), GetEvent(), NULL);
}

void CCalendarDemoView::OnCalendarMeetingOpen()
{
	CXTPCalendarEvent* pEvent = GetSelEvent();
	CXTPCalendarEventPtr ptrEvent = pEvent->CloneEvent();
...
void CCalendarDemoView::OnCalendarMeetingDelete()
{
	if (AfxMessageBox(_T("Delete the selected event?"), MB_YESNO | MB_ICONQUESTION) == IDYES)
	{
		CXTPCalendarData* pData = GetCalendarCtrl().GetDataProvider();
		if (pData)
		{
			pData->DeleteEvent(GetSelEvent());
		}
	}
}

void CCalendarDemoView::OnCategories(UINT nStyle)
{
	switch (nStyle)
	{
	case ID_CATEGORIES_ALLCATEGORIES:
		{
			CXTPCalendarEventColorDlg dlg(TRUE, &GetCalendarCtrl(), this);
			dlg.DoModal();
		}
		break;

	case ID_CATEGORIES_ALLLABELS:
		{
			CXTPCalendarEventColorDlg dlg(FALSE, &GetCalendarCtrl(), this);
			dlg.DoModal();
		}
		break;

	case ID_CATEGORIES_ALLCATEGORIESANDLABELS:
		{
			CCalendarEventColorDlg dlg(&GetCalendarCtrl(), this);
			dlg.DoModal();
		}
		break;
	}
...
BOOL CCalendarDemoView::IsNotRecurring(CXTPCalendarEvent* pEvent)
{
	return (xtpCalendarRecurrenceNotRecurring == pEvent->GetRecurrenceState());
}

void CCalendarDemoView::OnCalendarMeetingPrivate()
{
	CXTPCalendarEvent* pEvent = GetSelEvent();
	if (pEvent)
	{
		BOOL bPrivate = pEvent->IsPrivate();
		pEvent->SetPrivate(!bPrivate);

		CXTPCalendarControl::CUpdateContext updateContext(
			&GetCalendarCtrl(), xtpCalendarUpdateLayout | xtpCalendarUpdateRedraw);

		CXTPCalendarData* pData = GetCalendarCtrl().GetDataProvider();
		if (pData)
		{
			VERIFY(pData->ChangeEvent(pEvent));
		}
	}
}

void CCalendarDemoView::OnCalendarMeetingMeeting()
{
	CXTPCalendarEvent* pEvent = GetSelEvent();
	if (pEvent)
	{
		BOOL bMeeting = pEvent->IsMeeting();
		pEvent->SetMeeting(!bMeeting);
...
void CCalendarDemoView::OnCalendarMeetingReminder()
{
	CXTPCalendarEvent* pEvent = GetSelEvent();
	if (pEvent)
	{
		BOOL bReminder = pEvent->IsReminder();
		pEvent->SetReminder(!bReminder);
...
void CCalendarDemoView::OnUpdateCalendarMeetingPrivate(CCmdUI *pCmdUI)
{
	CXTPCalendarEvent* pEvent = GetSelEvent();
	if (pEvent)
	{
		pCmdUI->SetCheck(pEvent->IsPrivate());
		pCmdUI->Enable(IsNotRecurring(pEvent));
	}
}

void CCalendarDemoView::OnUpdateCalendarMeetingMeeting(CCmdUI *pCmdUI)
{
	CXTPCalendarEvent* pEvent = GetSelEvent();
	if (pEvent)
	{
		pCmdUI->SetCheck(pEvent->IsMeeting());
		pCmdUI->Enable(IsNotRecurring(pEvent));
	}
}

void CCalendarDemoView::OnUpdateCalendarMeetingReminder(CCmdUI *pCmdUI)
{
	CXTPCalendarEvent* pEvent = GetSelEvent();
	if (pEvent)
	{
		pCmdUI->SetCheck(pEvent->IsReminder());
		pCmdUI->Enable(IsNotRecurring(pEvent));
	}
}
