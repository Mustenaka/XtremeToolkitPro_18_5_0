...
void CDatePickerDemoDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_SHOW_3D_BORDER, m_checkBorder);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(CDatePickerDemoDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_RADIO_OFFICE2013, OnRadioOffice2013)
	//}}AFX_MSG_MAP
...
BOOL CDatePickerDemoDlg::OnInitDialog()
{
	...
	if (XTPSystemVersion()->IsWinXP())
	{
		...
		for(; i <= 7; ++i )
		{
			strDayName = m_wndDatePicker.GetDayOfWeekName(i);
			m_wndDatePicker.SetDayOfWeekName(i, strDayName.Left(2));
		}
	}
...
void CDatePickerDemoDlg::OnBnClickedBtnPopup()
{
	...
	wndDatePicker.SetPaintTheme(m_wndDatePicker.GetPaintTheme());
	CRect rc;
	...
	if (XTPSystemVersion()->IsWinXP())
	{
		...
		for(; i <= 7; ++i )
		{
			strDayName = wndDatePicker.GetDayOfWeekName(i);
			wndDatePicker.SetDayOfWeekName(i, strDayName.Left(2));
		}
	}
...
void CDatePickerDemoDlg::OnRadioOffice2007() 
{
	...
	m_wndDatePicker.SetPaintTheme(xtpCalendarThemeOffice2007);
	
	m_checkBorder.EnableWindow(TRUE);
	m_checkBorder.SetCheck(TRUE);
}

void CDatePickerDemoDlg::OnRadioOffice2013()
{
	UpdateData();

	m_wndDatePicker.SetPaintTheme(xtpCalendarThemeOffice2013);

	m_checkBorder.EnableWindow(FALSE);
	m_checkBorder.SetCheck(FALSE);
}
