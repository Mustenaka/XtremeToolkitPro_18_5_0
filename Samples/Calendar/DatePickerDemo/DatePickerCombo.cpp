		...
		if (wndDatePicker.GetSelRange(dtFrom, dtTo))
		{
			...
			} else
			{
				...
				strPopup.Format(_T("%s - %s"),
					CTime(dtFrom.GetYear(), dtFrom.GetMonth(), dtFrom.GetDay(), 0, 0, 0).Format(_T("%x")).operator LPCTSTR(), 
					CTime(dtTo.GetYear(), dtTo.GetMonth(), dtTo.GetDay(), 0, 0, 0).Format(_T("%x")).operator LPCTSTR());
