...
enum IDITEM
{
	...
	ID_ITEM_PIESTYLE_3D,
	ID_ITEM_DOUGHNUTSTYLE_3D,
	ID_ITEM_TORUSSTYLE_3D,
...
void CExplorerBar::Create(CWnd* pWndParent)
{
	...
	SetTheme(xtpTaskPanelThemeVisualStudio2012Light);
	...
	AddLinkItem(pGroup, ID_ITEM_PIESTYLE_3D, _T("3D Pie"));
	AddLinkItem(pGroup, ID_ITEM_DOUGHNUTSTYLE_3D, _T("3D Doughnut"));
	AddLinkItem(pGroup, ID_ITEM_TORUSSTYLE_3D, _T("3D Torus"));
	...
	AddLinkItem(pGroup, ID_ITEM_ANALYSIS, _T("Analysis"));
...
void CExplorerBar::OnItemClick(UINT nID)
{
	...
	case ID_ITEM_BARANDLINESTYLE:
	    ...
	    break;
	case ID_ITEM_ANALYSIS:
		pMainFrame->SetView(RUNTIME_CLASS(CAnalysisView));
		break;
