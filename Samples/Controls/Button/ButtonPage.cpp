...
void CButtonPage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_GBOX_CHECKBOXES, m_gboxCheckboxes);
	DDX_Control(pDX, IDC_GBOX_RADIO, m_gboxRadio);
	DDX_Control(pDX, IDC_GBOX_THEME, m_gboxTheme);
	DDX_Control(pDX, IDC_GBOX_JUSTIFICATION, m_gboxJustification);
	...
	DDX_Check(pDX, IDC_CHECK_DPI_SCALING, m_bImageDPIScaling);
...
BEGIN_MESSAGE_MAP(CButtonPage, CXTPResizePropertyPage)
	...
	ON_BN_CLICKED(IDC_CHECK_DPI_SCALING, OnBnClickedCheckDpiScaling)
...
void CButtonPage::AddTheme(LPCTSTR lpszThemeName, XTPControlTheme themeID, LPCTSTR lpszINI /*=NULL*/, BOOL bSelect /*=FALSE*/)
{
...
BOOL CButtonPage::OnInitDialog()
{
	...
	AddTheme(_T("Standard"),                   xtpControlThemeDefault);
	...
	AddTheme(_T("Office 2013 (Word)"),         xtpControlThemeOffice2013, xtpIniOffice2013Word);
	AddTheme(_T("Native Windows"),             xtpControlThemeNativeWinXP);
	AddTheme(_T("Visual Studio 2015 (Blue)"),  xtpControlThemeVisualStudio2015, xtpIniVisualStudio2015Blue, TRUE);
	AddTheme(_T("Visual Studio 2015 (Dark)"),  xtpControlThemeVisualStudio2015, xtpIniVisualStudio2015Dark);
	...
	for (i = IDC_GBOX_THEME; i <= IDC_CHECK_DPI_SCALING; i++)
...
void CButtonPage::OnCheckEnabled() 
{
	...
	m_gboxRegular.EnableWindow(m_bEnable);
	m_gboxTwoRows.EnableWindow(m_bEnable);
	m_gboxCheckboxes.EnableWindow(m_bEnable);
	m_gboxRadio.EnableWindow(m_bEnable);
}
...
void CButtonPage::OnSelChangeListTheme() 
{
	...
	if (pData)
	{
		...
		m_bUseVisualStyle = (pData->themeID == xtpControlThemeNativeWinXP);
		UpdateData(FALSE);
		OnCheckWinxpThemes();
...
void CButtonPage::OnSize(UINT nType, int cx, int cy) 
{
	CXTPResizePropertyPage::OnSize(nType, cx, cy);
	
	// TODO: Add your message handler code here
	
}

void CButtonPage::OnBnClickedCheckDpiScaling()
{
	m_bImageDPIScaling = !m_bImageDPIScaling;

	m_btnSettings.SetDpiImageScaling(m_bImageDPIScaling);
	m_btnFind.SetDpiImageScaling(m_bImageDPIScaling);
	m_btnHelp.SetDpiImageScaling(m_bImageDPIScaling);
	m_btnRun.SetDpiImageScaling(m_bImageDPIScaling);
