...
void CFontComboDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO_THEME, m_wndComboTheme);
	...
	DDX_Control(pDX, IDC_CHK_BOLD, m_btnBold);
	DDX_Control(pDX, IDC_CHK_ITALIC, m_btnItalic);
	DDX_Control(pDX, IDC_CHK_UNDERLINE, m_btnUnderline);
...
BEGIN_MESSAGE_MAP(CFontComboDlg, CDialog)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_CBN_SELENDOK(IDC_COMBO_THEME, OnThemeChanged)
...
BOOL CFontComboDlg::OnInitDialog()
{
	...
	m_wndComboFont.InitControl(m_strFontName);
...
void CFontComboDlg::SetTheme(XTPControlTheme theme)
{
	//nTheme = xtpControlThemeOffice2013;
	//nTheme = xtpControlThemeVisualStudio2012Light;
	//nTheme = xtpControlThemeVisualStudio2012Dark;

	SendMessageToDescendants(WM_XTP_SETCONTROLTHEME, theme);
	m_wndComboTheme.SetTheme(theme);
...
void CFontComboDlg::OnThemeChanged() 
{
	XTPControlTheme theme = xtpControlThemeDefault;
	switch(m_wndComboTheme.GetCurSel())
	{
	case 0: // Default
		theme = xtpControlThemeDefault;
		break;
	
	case 1: // Flat
		theme = xtpControlThemeFlat;
		break;
	
	case 2: // UltraFlat
		theme = xtpControlThemeUltraFlat;
		break;
	
	case 3: // Office 2000
		theme = xtpControlThemeOffice2000;
		break;
	
	case 4: // Office XP
		theme = xtpControlThemeOfficeXP;
		break;
	
	case 5: // Office 2003
		theme = xtpControlThemeOffice2003;
		break;

	case 6: // Office 2013 (Word)
		theme = xtpControlThemeOffice2013;
		XTPThemeDLL()->SetHandle(xtpIniOffice2013Word);
		break;

	case 7: // Office 2013 (Access)
		theme = xtpControlThemeOffice2013;
		XTPThemeDLL()->SetHandle(xtpIniOffice2013Access);
		break;

	case 8: // Windows XP
		theme = xtpControlThemeNativeWinXP;
		break;
	
	case 9: // Visual Studio 2005
		theme = xtpControlThemeVisualStudio2005;
		break;
	
	case 10: // Visual Studio 2008
		theme = xtpControlThemeVisualStudio2008;
		break;
	
	case 11: // Visual Studio 2010
		theme = xtpControlThemeVisualStudio2010;
		break;

	case 12: // Visual Studio 2012 (Light)
		theme = xtpControlThemeVisualStudio2012Light;
		break;

	case 13: // Visual Studio 2012 (Dark)
		theme = xtpControlThemeVisualStudio2012Dark;
		break;

	case 14: // Visual Studio 2015 (Light)
		theme = xtpControlThemeVisualStudio2015;
		XTPThemeDLL()->SetHandle(xtpIniVisualStudio2015Light);
		break;

	case 15: // Visual Studio 2015 (Dark)
		theme = xtpControlThemeVisualStudio2015;
		XTPThemeDLL()->SetHandle(xtpIniVisualStudio2015Dark);
		break;

	case 16: // Visual Studio 2015 (Blue)
		theme = xtpControlThemeVisualStudio2015;
		XTPThemeDLL()->SetHandle(xtpIniVisualStudio2015Blue);
		break;

	default:
		ASSERT(!"Unknown theme selected");
		break;
	}
