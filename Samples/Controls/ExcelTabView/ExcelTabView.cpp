...
BEGIN_MESSAGE_MAP(CExcelTabView, CView)
    ...
    ON_MESSAGE(WM_XTP_SETCONTROLTHEME, OnSetControlTheme)
END_MESSAGE_MAP()
...
LRESULT CExcelTabView::OnSetControlTheme(WPARAM wParam, LPARAM /*lParam*/)
{
    m_iTheme = (XTPControlTheme)wParam;

	switch (m_iTheme)
	{
	case xtpControlThemeOffice2013:
		m_bSquareTabs = TRUE;
		m_pExcelTabCtrl->SetTabHeight(XTP_DPI_Y(27));
		UpdateExcelTab();
		ModifyStyleEx(WS_EX_CLIENTEDGE, 0);
		break;
	default:
		m_bSquareTabs = FALSE;
		m_pExcelTabCtrl->SetTabHeight(::GetSystemMetrics(SM_CYHSCROLL));
		UpdateExcelTab();
...
BOOL CExcelTabView::CreateSheet1()
{
	...
	CXTPLogFont lf;
	XTPAuxData().xtpFont.GetLogFont(&lf);
...
BOOL CExcelTabView::CreateSheet4()
{
	...
	m_sheet4.SetFont(&XTPAuxData().xtpFont);
...
BOOL CExcelTabView::CreateSheet5()
{
	...
	m_sheet5.SetFont(&XTPAuxData().xtpFont);
...
BOOL CExcelTabView::CreateSheet6()
{
	...
	m_sheet6.SetFont(&XTPAuxData().xtpFont);
...
{
	CExcelTabProperties dlg(m_bBottom, m_bHasArrows, m_bHasHomeEnd, m_bHScroll, m_bSquareTabs, this);
	if (dlg.DoModal() == IDOK)
	{
	}	
}
...
void CExcelTabView::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS FAR* lpncsp) 
{
	if (m_iTheme == xtpControlThemeOffice2013)
	{
...
void CExcelTabView::OnNcPaint() 
{
	if (m_iTheme == xtpControlThemeOffice2013)
	{
		...
		COLORREF crBorder = XTPIniColor(_T("TabClient"), _T("Border"), GetXtremeColor(COLOR_3DSHADOW));

		dc.Draw3dRect(0, 0, cx, cy, crBorder, crBorder);
	}
	else
	{
		CView::OnNcPaint();
