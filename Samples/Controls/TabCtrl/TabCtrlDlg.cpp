...
void CEditListBoxEx::OnWindowPosChanging(WINDOWPOS FAR* lpwndpos)
{
	...
	{
		lpwndpos->cy -= XTP_DPI_Y(BTN_OFFSET);
		lpwndpos->y += XTP_DPI_Y(BTN_OFFSET);
	}
...
BOOL CTabCtrlDlg::OnInitDialog()
{
	...
	::SetWindowPos( GetSafeHwnd(), NULL, 0, 0, 1024, 768,
		SWP_FRAMECHANGED);
...
int CTabCtrlDlg::InitTabCtrl()
{
	...
	*/
	...
	m_tabCtrl.SetPadding(XTP_DPI(CSize(6, 3)));
...
int CTabCtrlDlg::InitEditList()
{
	...
	m_listBox.SetFont (&XTPAuxData().xtpFont);
