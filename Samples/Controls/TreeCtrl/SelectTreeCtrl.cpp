...
BEGIN_MESSAGE_MAP(CSelectTreeCtrl, CXTPTreeCtrl)
...
{
	CXTPTreeCtrl::OnSetFocus(pOldWnd);
...
{
	CXTPTreeCtrl::OnKillFocus(pNewWnd);
...
void CSelectTreeCtrl::PreSubclassWindow() 
{
	...
	int nOverlayIconIndex = m_imageList.Add(AfxGetApp()->LoadIcon(IDI_ICON_OVERLAY));
	m_imageList.SetOverlayImage(nOverlayIconIndex, 1);
