...
void CPropertiesDialog::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_BIGBALOON, m_btnBigBalloon);
...
BEGIN_MESSAGE_MAP(CPropertiesDialog, CXTPResizeDialog)
	...
	ON_BN_CLICKED(IDC_CHECK_BIGBALOON, OnBigBalloonClick)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BOOL CPropertiesDialog::OnInitDialog()
{
	...
	SetResize(IDC_CHECK_BIGBALOON,   XTP_ANCHOR_BOTTOMLEFT, XTP_ANCHOR_BOTTOMLEFT);

	m_btnBigBalloon.EnableWindow(XTPSystemVersion()->IsWinVistaOrGreater());

	// Load window placement
	LoadPlacement(_T("CPropertiesDialog"));

	m_txtBalloonTip.SetFont(&XTPAuxData().xtpFontBold);
	EnableControls();
...
void CPropertiesDialog::OnBtnShowBalloon()
{
	...
	HICON hIconBaloon = NULL; 

	DWORD dwInfoFlags = (m_iBalloonIcon < 4) ? balloonIcon[m_iBalloonIcon] : (NIIF_USER);

	if (m_btnBigBalloon.IsWindowEnabled() && m_btnBigBalloon.GetCheck())
	{
		hIconBaloon = static_cast<HICON>(LoadImage(AfxGetInstanceHandle(), 
			MAKEINTRESOURCE(IDR_MAINFRAME), IMAGE_ICON, GetSystemMetrics( SM_CXICON), GetSystemMetrics(SM_CYICON), LR_DEFAULTCOLOR));
	...
	pMainFrame->m_TrayIcon.ShowBalloonTip(m_strBalloonMsg, m_strBalloonTitle, dwInfoFlags, m_iTimeOut, hIconBaloon);
}
...
void CPropertiesDialog::OnBigBalloonClick()
{
	m_comboBalloonIcon.EnableWindow(!m_btnBigBalloon.GetCheck());
	m_txtBalloonIcon.EnableWindow(!m_btnBigBalloon.GetCheck());
}
