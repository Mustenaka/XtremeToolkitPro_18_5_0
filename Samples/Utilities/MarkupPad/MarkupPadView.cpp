...
CMarkupPadView::CMarkupPadView()
{
	...
	m_fntRtlMessage.CreatePointFont(XTP_DPI_X(80), _T("Arial"));
...
void CMarkupPadView::CreateCodeMarkup()
{
	...
	pBorder->SetMargin(XTP_DPI_X(5), XTP_DPI_Y(5), XTP_DPI_X(5), XTP_DPI_Y(5));
	pBorder->SetPadding(XTP_DPI_X(5), XTP_DPI_Y(5), XTP_DPI_X(5), XTP_DPI_Y(5));
	pBorder->SetBorderThickness(XTP_DPI_X(10), XTP_DPI_Y(10), XTP_DPI_X(10), XTP_DPI_Y(10));
	...
	pBorder2->SetMargin(XTP_DPI_X(1), XTP_DPI_Y(1), XTP_DPI_X(1), XTP_DPI_Y(1));
	pBorder2->SetPadding(XTP_DPI_X(8), XTP_DPI_Y(8), XTP_DPI_X(8), XTP_DPI_Y(8));
	pBorder2->SetBorderThickness(XTP_DPI_X(1), XTP_DPI_Y(1), XTP_DPI_X(1), XTP_DPI_Y(1));
	...
	pBorder3->SetPadding(XTP_DPI_X(10), XTP_DPI_Y(10), XTP_DPI_X(10), XTP_DPI_Y(10));
	...
	pTextBlock1->SetMargin(XTP_DPI_X(5), 0, XTP_DPI_X(5), 0);
	...
	pBorder4->SetBorderThickness(XTP_DPI_X(1), XTP_DPI_Y(1), XTP_DPI_X(1), XTP_DPI_Y(1));
	...
	pBorder5->SetWidth(XTP_DPI_X(10));
	pBorder5->SetHeight(XTP_DPI_Y(10));
	//pBorder5->SetMargin(8);
	pBorder5->SetBorderThickness(XTP_DPI_X(1), XTP_DPI_Y(1), XTP_DPI_X(1), XTP_DPI_Y(1));
...
void CMarkupPadView::DrawMarkup(CXTPMarkupDrawingContext* pDC, CRect rc)
{
	...
	CDC* pCDC = CDC::FromHandle(pDC->GetDC());
	if (LAYOUT_RTL == pCDC->GetLayout())
	{
		CXTPDCSaveState cdState(pCDC);
		pCDC->SetBkMode(TRANSPARENT);
		pCDC->SetTextColor(RGB(0x80, 0x80, 0x80));
		pCDC->SelectObject(&m_fntRtlMessage);
		pCDC->DrawText(_T("Markup rendering capabilities are limited in RTL mode\n")
			_T("Transformations and animations are disabled"),
			&rc, DT_RIGHT | DT_TOP);
	}

	pDC->ReleaseDC(pCDC->m_hDC);
}
