...
void CResourceEditorView::OnReportValueChanged(NMHDR * pNotifyStruct, LRESULT * /*result*/)
{
	...
	{
		CResourceViewRecord* pRecord = DYNAMIC_DOWNCAST(CResourceViewRecord, pItemNotify->pItem->GetRecord());
		if (pRecord == NULL)
		{
			return;
		}
...
void CResourceEditorView::OnReportSelectionChanged(NMHDR * /*pNotifyStruct*/, LRESULT * /*result*/)
{
	...
	{
		CResourceViewRecord* pRecord = DYNAMIC_DOWNCAST(CResourceViewRecord, pRow->GetRecord());
		if (pRecord == NULL)
		{
