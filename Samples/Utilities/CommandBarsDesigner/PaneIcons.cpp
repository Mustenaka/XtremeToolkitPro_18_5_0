...
#define THUMB_WIDTH XTP_DPI_X(90 - 20)
#define THUMB_HEIGHT XTP_DPI_Y(100 - 20)
#define INFO_HEIGHT XTP_DPI_Y(15)
...
void CControlIconsViewItem::Draw(CDC* pDC, BOOL bSelected, int x, int y, CSize sz)
{
	...
	rcDevice.top = rcPicture.bottom - szDevice.cy - XTP_DPI_Y(4);
	...
	rcImage.DeflateRect(XTP_DPI_X(4), XTP_DPI_Y(4), 
		XTP_DPI_X(4), rcDevice.Height());
	...
	ASSERT(!hIcon.IsEmpty());
	{
		...
		rcImageRect.InflateRect(XTP_DPI_X(1), XTP_DPI_Y(1));
...
CControlIconsViewItem* CControlIconsView::HitTest(CPoint pt)
{
	...
	while (pos)
	{
		...
		CRect rc(CPoint(-GetScrollOffset(SB_HORZ) + XTP_DPI_X(3) + item.ptPosition.x * THUMB_WIDTH, 
			-GetScrollOffset(SB_VERT) + XTP_DPI_Y(3) + item.ptPosition.y * THUMB_HEIGHT), 
			CSize(THUMB_WIDTH - XTP_DPI_X(5), THUMB_HEIGHT - XTP_DPI_Y(5)));
...
void CControlIconsView::OnPaint()
{
	...
	while (pos)
	{
		...
		item.Draw(&dc, m_pSelected == &item, -GetScrollOffset(SB_HORZ) + XTP_DPI_X(3) + item.ptPosition.x * THUMB_WIDTH, 
			-GetScrollOffset(SB_VERT) + XTP_DPI_Y(3) + item.ptPosition.y * THUMB_HEIGHT, 
			CSize(THUMB_WIDTH - XTP_DPI_X(5), THUMB_HEIGHT - XTP_DPI_Y(5)));
...
void CControlIconsView::UpdateScrollBars()
{
	...
	int nHeight = XTP_DPI_Y(3) + nCount * THUMB_HEIGHT;
	CXTPClientRect rc(this);
	...
	int nWidth = m_lstItems.GetCount() > 0 ? XTP_DPI_X(3) + nStateCount * THUMB_WIDTH: 0;
...
int CPaneIcons::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_wndToolBar.SetIconSize(XTP_DPI(CSize(16, 16)));
	...
	m_wndCombo.Create(WS_VISIBLE | WS_CHILD | CBS_DROPDOWNLIST, CRect(0, 0, 0, XTP_DPI_Y(200)), this, ID_PANEICONS_COMBO);
