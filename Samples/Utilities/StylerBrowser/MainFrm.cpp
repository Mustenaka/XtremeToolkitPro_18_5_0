...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_wndStatusBar.SetPaneInfo(1, ID_INDICATOR, SBPS_NORMAL, XTP_DPI_X(95));

	m_wndStatusBar.AddIndicator(ID_EMPTY);
	m_wndStatusBar.SetPaneInfo(2, ID_EMPTY, SBPS_NORMAL, XTP_DPI_X(245));
	...
	CXTPDockingPane* A = m_paneManager.CreatePane(IDS_TITLE_HIS, XTP_DPI(CRect(0, 0,200, 120)), xtpPaneDockLeft);
	CXTPDockingPane* B = m_paneManager.CreatePane(IDS_TITLE_SHELL, XTP_DPI(CRect(0, 0, 200, 120)), xtpPaneDockBottom, A);
	CXTPDockingPane* C = m_paneManager.CreatePane(IDS_TITLE_NOTEPAD, XTP_DPI(CRect(0, 0, 200, 120)), xtpPaneDockRight);
	CXTPDockingPane* D = m_paneManager.CreatePane(IDS_TITLE_SOURCE, XTP_DPI(CRect(0, 0, 200, 120)), xtpPaneDockBottom, C);
	CXTPDockingPane* E = m_paneManager.CreatePane(IDS_TITLE_SEARCH, XTP_DPI(CRect(0, 0, 200, 120)), xtpPaneDockBottom, D);
...
void CMainFrame::UninitialNameSpace()
{
	if(m_pSession!=NULL)
	{
		...
		ASSERT(m_pFilter);
		DWORD refCount = m_pFilter->InternalRelease();
		while (refCount > 0)
			refCount = m_pFilter->InternalRelease();
		m_pFilter = NULL;
	}
}
...
void CMainFrame::SetTheme(XTPPaintTheme theme)
{
	...
	m_paneManager.SetTheme(
		...
		m_themeCurrent == xtpThemeVisualStudio2005 ? xtpPaneThemeVisualStudio2005: 
			xtpPaneThemeWinNative);

	CXTPPaintManager::SetTheme(theme);

	EnableFrameTheme(theme == xtpThemeRibbon || theme == xtpThemeResource ? GetCommandBars() : 0);
	...
	if (m_themeCurrent == xtpThemeRibbon)
	{
		...
		m_MTIClientWnd.GetPaintManager()->m_rcButtonMargin.SetRect(
			XTP_DPI_X(2), XTP_DPI_Y(2), XTP_DPI_X(2), XTP_DPI_Y(2));
	}
...
int CMainFrame::OnCreateControl(LPCREATECONTROLSTRUCT lpCreateControl)
{
	...
	if (lpCreateControl->bToolBar)
	{
		...
		if (lpCreateControl->nID == ID_GOTO_URL && pToolBar->GetBarID() == IDR_MAINFRAME)
		{
			...
			pComboAddress->SetWidth(XTP_DPI_X(350));
		...
		if (lpCreateControl->nID == ID_GOTO_SEARCH && pToolBar->GetBarID() == IDR_SEARCH)
		{
			...
			pComboSearch->SetWidth(XTP_DPI_X(150));
