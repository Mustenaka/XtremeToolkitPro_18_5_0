...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	CXTPDockingPane* pPaneClassView = m_paneManager.CreatePane(
		IDR_PANE_CLASSVIEW, XTP_DPI(CRect(0, 0,250, 120)), xtpPaneDockLeft);

	CXTPDockingPane* pPaneResourceView = m_paneManager.CreatePane(
		IDR_PANE_RESOURCEVIEW, XTP_DPI(CRect(0, 0, 200, 120)), xtpPaneDockBottom, pPaneClassView);

	CXTPDockingPane* pPaneFileView = m_paneManager.CreatePane(
		IDR_PANE_FILEVIEW, XTP_DPI(CRect(0, 0, 200, 120)), xtpPaneDockBottom, pPaneClassView);

	CXTPDockingPane* pPaneInfoView = m_paneManager.CreatePane(
		IDR_PANE_INFOVIEW, XTP_DPI(CRect(0, 0, 200, 120)), xtpPaneDockBottom, pPaneClassView);
	...
	m_paneManager.GetPaintManager()->GetTabPaintManager()->m_rcControlMargin.SetRect(
		XTP_DPI_X(3), XTP_DPI_Y(3), XTP_DPI_X(3), XTP_DPI_Y(3));
		...
		if (!pPane->IsValid())
		{
			...
			if (!::IsWindow(m_wndPanes[nIndex].m_hWnd))
			{
				if (!m_wndPanes[nIndex].CreateEx(WS_EX_CLIENTEDGE, _T("EDIT"), _T(""),
					WS_CHILD | ES_AUTOVSCROLL | ES_MULTILINE, XTP_DPI(CRect(0, 0, 200, 120)), this, 0))
				{
