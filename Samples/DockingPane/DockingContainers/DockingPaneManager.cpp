...
void CDockingPanePaintManager::DrawAboutGlyph(CDC* pDC, CPoint pt, COLORREF clr)
{
	pt.Offset(-m_nTitleHeight / 4, -m_nTitleHeight / 4);
	CRect rc(pt, CSize(m_nTitleHeight / 2, m_nTitleHeight / 2));
	XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolQuestionMark, rc, clr);
}
...
void CDockingPanePaintManager::RoundRect(CDC* pDC, CRect rc, COLORREF clr)
{
	...
	pDC->RoundRect(rc.left, rc.top, rc.right, rc.bottom, XTP_DPI_X(6), XTP_DPI_Y(6));
}
...
void CDockingPanePaintManager::DrawCaptionButtonEntry(CDC* pDC, CXTPDockingPaneCaptionButton* pButton, CPoint pt, COLORREF clrButton)
{
	if (pButton->GetID() == IDS_ABOUT)
	{
		DrawAboutGlyph(pDC, pt, clrButton);
	}
	else
	{
...
void CDockingPanePaintManager::DrawSplitter(CDC& dc, CXTPDockingPaneSplitterWnd* pSplitter)
{
	...
	{
		for (int y = 1; y < rc.Height() - 1; y += XTP_DPI_Y(4))
		{
			dc.FillSolidRect(XTP_DPI_X(3), y + XTP_DPI_Y(1), XTP_DPI_X(2), XTP_DPI_Y(2), GetXtremeColor(COLOR_BTNHIGHLIGHT));
			dc.FillSolidRect(XTP_DPI_X(2), y, XTP_DPI_X(2), XTP_DPI_Y(2), GetXtremeColor(XPCOLOR_TOOLBAR_GRIPPER));
		}
	}
	else
	{
		for (int x = 1; x < rc.Width() - 1; x += XTP_DPI_X(4))
		{
			dc.FillSolidRect(x + XTP_DPI_X(1), XTP_DPI_Y(3), XTP_DPI_X(2), XTP_DPI_Y(2), GetXtremeColor(COLOR_BTNHIGHLIGHT));
			dc.FillSolidRect(x, XTP_DPI_Y(2), XTP_DPI_X(2), XTP_DPI_Y(2), GetXtremeColor(XPCOLOR_TOOLBAR_GRIPPER));
		}
	}
